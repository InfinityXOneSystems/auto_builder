...
def normalize_file(compiled_path, source_path, out_dir, sample_limit=SAMPLE_ROWS_PER_FILE):
    ...
    with compiled.open('r', encoding='utf-8', errors='replace') as fh:
        ...
        for r in reader:
            ...
            if not text:
                possible = []
                for k in header:
                    if re.search(r'(paragraph|line|sentence|content|body|text)', k, re.I):
            ...
            if not (url or title or text):
                continue
            rows_out.append({'source': source_path, 'orig': str(compiled), 'url': url, 'title': title, 'text': text, 'date': date, 'score': score})
    ...
    with out_file.open('w', encoding='utf-8', newline='') as outfh:
        writer = csv.DictWriter(outfh, fieldnames=['source', 'orig', 'url', 'title', 'text', 'date', 'score'])
        writer.writeheader()
        for r in rows_out:
            writer.writerow(r)
...
def main():
    ...
    consolidated_writer = csv.DictWriter(consolidated_fh, fieldnames=['source', 'orig', 'url', 'title', 'text', 'date', 'score'])
    ...
    for item in manifest:
        ...
        if rows_out > 0:
            ...
            with (NORMALIZED_DIR / (compiled_path.stem + '_normalized.csv')).open('r', encoding='utf-8') as nf:
