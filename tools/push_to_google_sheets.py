...
def ensure_packages():
    try:
        ...
        print('  pip install gspread google-auth google-auth-oauthlib google-auth-httplib2 gspread-formatting')
        sys.exit(2)
...
def get_gspread_client():
    ...
    print('No credentials found. Place a service account JSON at .secrets/service_account.json or set env GOOGLE_SERVICE_ACCOUNT_JSON=path')
    print('Or put OAuth client secrets at .secrets/credentials.json for an interactive flow.')
    sys.exit(3)
...
def write_sheet_from_rows(client, spreadsheet_id, rows, sheet_title='Signals'):
    ...
    try:
        ...
        pass
    worksheet = sh.add_worksheet(title=sheet_title, rows=str(max(100, len(rows)+5)), cols='20')
    ...
    try:
        ...
        format_cell_ranges(worksheet, [('A1:{}1'.format(chr(65 + len(header) - 1)), fmt)])
