...
所有编译器选项,“{0}”修饰符不能与导入声明一起使用。,“{0}”参数必须是第一个参数。,JSDoc“@template”标记不能跟在“@typedef”、“@callback”或“@overload”标记后面,"JSDoc ""@typedef"" 注释不能包含多个 ""@type"" 标记。",“bigint”文本不能用作属性名称。,BigInt 字面量中不能使用指数符号。,BigInt 字面量必须是整数。,绑定模式参数在实现签名中不能为可选参数。,"""break"" 语句只能在封闭迭代或 switch 语句内使用。","""break"" 语句只能跳转到封闭语句的标签。",字符类不得包含保留的双标点符号。你是否想要使用反斜杠对其进行转义?,字符类范围不能由另一个字符类绑定。,类只能实现具有可选类型参数的标识符/限定名称。,类只能实现具有静态已知成员的对象类型或对象类型的交集。,类无法扩展“{0}”这样的基元类型。类只能扩展可构造值。,类无法实现类似于“{0}”的基元类型。它只能实现其他命名对象类型。,"不带 ""default"" 修饰符的类声明必须具有名称。",类成员不可具有“{0}”关键字。,计算属性名中不允许逗号表达式。,计算属性名无法从其包含的类型引用类型参数。,类属性声明中的计算属性名称必须具有简单文本类型或“唯一符号”类型。,"方法重载中的计算属性名称必须引用文本类型或 ""unique symbol"" 类型的表达式。","类型文本中的计算属性名称必须引用类型为文本类型或 ""unique symbol"" 类型的表达式。","环境上下文中的计算属性名称必须引用类型为文本类型或 ""unique symbol"" 类型的表达式。","接口中的计算属性名称必须引用必须引用类型为文本类型或 ""unique symbol"" 的表达式。","计算属性名的类型必须为 ""string""、""number""、""symbol"" 或 ""any""。",'const' 断言只能作用于枚举成员、字符串、数字、布尔值、数组或对象字面量。,只有使用字符串文本才能访问常数枚举成员。,"环境上下文中的 ""const"" 初始化表达式必须为字符串、数字文本或文本枚举引用。","当构造函数的类扩展 ""null"" 时，它不能包含 ""super"" 调用。","构造函数不可具有 ""this"" 参数。","""continue"" 语句只能在封闭迭代语句内使用。","""continue"" 语句只能跳转到封闭迭代语句的标签。",如果没有“导入类型”，则无法导入声明文件。是否要改为导入实现文件“{0}”?,"不能在已有的环境上下文中使用 ""declare"" 修饰符。",修饰器仅可修饰方法实现，而不可修饰重载。,"""default"" 子句在 ""switch"" 语句中只能出现一次。",默认导出只能在 ECMAScript-style 模块中使用。,默认导出必须位于文件或模块声明的顶层。,"此上下文中不允许明确的赋值断言 ""!""。",析构声明必须具有初始化表达式。,ES5 中的动态导入调用需要“Promise”构造函数。请确保对 “Promise” 构造函数进行了声明或在 “--lib” 选项中包含了 “ES2015”。,动态导入调用返回 “Promise”。请确保具有对 “Promise” 的声明或在 “--lib” 选项中包含了 “ES2015”。,文件不能引用自身。,返回“从不”的函数不能具有可访问的终结点。,使用 'new' 关键字调用的函数的 'this' 类型不能为 'void'。,"其声明类型不为 ""undefined""、""void"" 或 ""any"" 的函数必须返回值。","生成器不能具有 ""void"" 类型批注。","""get"" 访问器不能具有参数。",Get 访问器必须至少具有与 Setter 相同的可访问性,"""get"" 访问器必须返回值。",此处不允许使用 'A 标签。,标记的元组元素被声明为可选，并且问号位于名称之后、冒号之前，而不是位于类型之后。,标记的元组元素通过在名称之前(而不是类型之前)的 “...” 声明为 rest。,映射的类型可能不声明属性或方法。,枚举声明中的成员初始化表达式不能引用在其后声明的成员(包括在其他枚举中定义的成员)。,"mixin 类必须具有单个 rest 参数为类型 ""any[]"" 的构造函数。","从包含抽象构造签名的类型变量扩展的 mixin 类也必须声明为 ""abstract""。",一个模块不能具有多个默认导出。,命名空间声明必须位于与之合并的类或函数所在的相同文件内。,命名空间声明不能位于与之合并的类或函数前,命名空间声明只允许位于命名空间或模块的顶层。,"不应使用 ""module"" 关键字声明 ""namespace"" 声明。请改用 ""namespace"" 关键字。",非 -dry 生成将生成项目“{0}”,非 -dry 生成将删除以下文件: {0},非 dry 生成将更新项目 '{0}' 的输出的时间戳,只允许在函数或构造函数实现中使用参数初始化表达式。,不能使用 rest 参数声明参数属性。,只允许在构造函数实现中使用参数属性。,不能使用绑定模式声明参数属性。,'Promise' 必须具有 'then' 方法。,"类型为 ""unique symbol"" 的类的属性必须同时为 ""static"" 和 ""readonly""。","类型为 ""unique symbol"" 的接口或类型文本的属性必须为 ""readonly""。",必选元素不能位于可选元素后。,必选参数不能位于可选参数后。,rest 元素不能包含绑定模式。,rest 元素不能跟在另一个 rest 元素之后。,其余元素不能具有属性名。,rest 元素不能具有初始化表达式。,rest 元素必须在析构模式中位于最末。,rest 元素类型必须是数组类型。,rest 参数不能为可选参数。,rest 参数不能具有初始化表达式。,rest 参数必须是参数列表中的最后一个参数。,rest 参数必须是数组类型。,Rest 参数或绑定模式不可带尾随逗号。,"""return"" 语句只能在函数体中使用。",不能在类静态块内使用 “return” 语句。,"一系列条目，这些条目将重新映射导入内容，以查找与 ""baseUrl"" 有关的位置。","""set"" 访问器不能具有返回类型批注。","""set"" 访问器不能具有可选参数。","""set"" 访问器不能具有 rest 参数。","""set"" 访问器必须正好具有一个参数。","""set"" 访问器参数不能包含初始化表达式。",扩张参数必须具有元组类型或传递给 rest 参数。,“super” 调用必须是包含初始化属性、参数属性或专用标识符的派生类的构造函数中的根级语句。,当派生类包含初始化属性、参数属性或专用标识符时，“super” 调用必须是构造函数中用来引用 “super” 或 “this” 的第一个语句。,"基于 ""this"" 的类型防护与基于参数的类型防护不兼容。","""this"" 类型仅在类或接口的非静态成员中可用。",启用“verbatimModuleSyntax”时，不能对 CommonJS 模块中的值声明使用顶级“export”修饰符。,"已在“{0}”中定义了 ""tsconfig.json"" 文件。",元组成员不能既是可选的又是 rest。,不能使用负值为元组类型编制索引。,乘方表达式的左侧不允许出现类型断言表达式。请考虑用括号将表达式括起。,类型文字数据不可具有初始化表达式。,仅类型导入可以指定默认导入或命名绑定，但不能同时指定这两者。,类型谓词无法引用 rest 参数。,类型谓词无法在绑定模式中引用元素“{0}”。,只允许在函数和方法的返回类型位置使用类型谓词。,类型谓词的类型不可赋给其参数的类型。,启用 “isolatedModules” 和 “emitDecoratorMetadata” 时，必须使用 “import type” 或命名空间导入来导入修饰签名中引用的类型。,"类型为 ""unique symbol"" 的变量必须为 ""const""。","只允许在生成器正文中使用 ""yield"" 表达式。",无法通过 super 表达式访问“{1}”类中的“{0}”抽象方法。,抽象方法只能出现在抽象类中。,抽象属性只能出现在抽象类中。,不能在构造函数中访问类“{1}”中的抽象属性“{0}”。,已看到可访问性修饰符。,访问器仅在面向 ECMAScript 5 和更高版本时可用。,两个取值函数必须都是抽象的或都是非抽象的。,将“{0}.”添加到未解析的变量,添加 return 语句,将返回类型添加到函数声明。,将返回类型添加到函数表达式。,将返回类型添加到 get 访问器声明。,向方法添加返回类型,向参数 {0} 添加类型注释。,向属性 {0} 添加类型注释。,向变量 {0} 添加类型注释。,将类型添加到 set 访问器声明的参数。,"添加所有缺失的 ""async"" 修饰符",添加所有缺少的属性,添加所有缺失的调用括号,添加所有缺少的函数声明,添加所有缺少的导入,添加所有缺少的成员,"添加所有缺失的 ""override"" 修饰符",添加所有缺少的参数,添加所有缺少的属性,添加所有缺少的 return 语句,添加所有缺失的 super() 调用,添加所有缺少的类型注释,添加所有可选参数,添加类型为“{0}”的注释,将异步修饰符添加到包含函数,"添加 ""await""","将 ""await"" 添加到 ""{0}"" 的初始值设定项","将 ""await"" 添加到初始值设定项",向箭头函数添加大括号,"将 ""const"" 添加到所有未解析变量","将 ""const"" 添加到未解析的变量",向属性“{0}”添加明确的赋值断言,将明确赋值断言添加到未初始化的所有属性,"添加 ""export {}""，将此文件变为模块",添加 `extends` 约束。,将 `extends` 约束添加到所有类型参数,从“{0}”添加导入,为属性“{0}”添加索引签名,向属性“{0}”添加初始值设定项,将初始化表达式添加到未初始化的所有属性,添加缺少的属性,添加缺失的调用括号,为对象成员完成“{0}”添加缺少的逗号。,"添加缺少的枚举成员 ""{0}""","添加缺少的函数声明 ""{0}""","将缺少的 ""new"" 运算符添加到所有调用","将缺少的 ""new"" 运算符添加到调用",将缺少的参数添加到“{0}”,将缺少的参数添加到“{0}”,添加缺少的属性,"添加缺失的 ""super()"" 调用","添加缺少的 ""typeof""",为没有名称的所有参数添加名称,将可选参数添加到“{0}”,将可选参数添加到“{0}”,添加或删除箭头函数中的大括号,"添加 ""override"" 修饰符",添加参数名称,将限定符添加到匹配成员名称的所有未解析变量,"添加 ""resolution-mode"" 导入属性","向所有需要 ""resolution-mode"" 导入属性的仅类型导入添加该属性",添加返回类型“{0}”,将 satisfies 和类型断言添加到此表达式 (satisfies T as T) 以使类型显式。,使用“{0}”添加 satisfies 和内联类型断言,"将 ""()"" 添加到所有未调用的修饰器","将 ""@ts-ignore"" 添加到所有错误消息",使用索引访问时，将 “undefined” 添加到类型。,将 “undefined” 添加到可选属性类型,将未定义的类型添加到未初始化的所有属性,"向属性“{0}”添加 ""undefined"" 类型","为非重叠类型添加 ""unknown"" 转换","将 ""unknown"" 添加到非重叠类型的所有转换","将 ""void"" 添加到已解析但没有值的 Promise","将 ""void"" 添加到所有已解析但没有值的 Promise",添加 tsconfig.json 文件有助于组织包含 TypeScript 和 JavaScript 文件的项目。有关详细信息，请访问 https://aka.ms/tsconfig。,"""{0}"" 的所有声明必须具有相同的限制。",“{0}”的所有声明必须具有相同的修饰符。,“{0}”的所有声明都必须具有相同的类型参数。,抽象方法的所有声明必须是连续的。,所有解构出的成员都未使用。,导入声明中的所有导入都未使用。,未使用任何类型参数。,所有变量均未使用。,允许 JavaScript 文件成为程序的一部分。使用 ''checkJs'' 选项来获取这些文件中的错误。,允许从模块访问 UMD 变量全局。,允许从不带默认输出的模块中默认输入。这不会影响代码发出，只是类型检查。,当模块没有默认导出时，允许“从 y 导入 x”。,允许每个项目从 tslib 导入帮助程序函数一次，而不是将它们包含在每个文件中。,允许导入包含 TypeScript 文件扩展名。需要设置“--moduleResolution bundler”以及“--noEmit”或“--emitDeclarationOnly”。,允许编译 JavaScript 文件。,允许在解析模块时将多个文件夹视为一个文件夹。,"已包含的文件名 ""{0}"" 仅大小写与文件名 ""{1}"" 不同。",环境模块声明无法指定相对模块名。,环境模块不能嵌套在其他模块或命名空间中。,AMD 模块无法拥有多个名称分配。,抽象访问器不能有实现。,可访问性修饰符不能与专用标识符一起使用。,访问器不能具有类型参数。,"""accessor"" 属性不能声明为可选。",只允许在文件的顶层中使用环境模块声明。,"未提供 ""{0}"" 的自变量。",未提供与此绑定模式匹配的自变量。,"算术操作数必须为类型 ""any""、""number""、""bigint"" 或枚举类型。","箭头函数不能包含 ""this"" 参数。",ES5 中的异步函数或方法需要“Promise”构造函数。请确保对 “Promise” 构造函数进行了声明或在 “--lib” 选项中包含了 “ES2015”。,异步函数或方法必须返回 “Promise”。请确保具有对 “Promise” 的声明或在 “--lib” 选项中包含了 “ES2015”。,"异步迭代器必须具有 ""next()"" 方法。",元素访问表达式应采用参数。,不能用专用标识符命名枚举成员。,枚举成员不能具有数值名。,"枚举成员名称的后面必须跟有 "",""、""="" 或 ""}""。",此信息的扩展版本，显示所有可能的编译器选项,不能在具有其他导出元素的模块中使用导出分配。,不能在命名空间中使用导出分配。,导出分配不能具有修饰符。,导出分配必须位于文件或模块声明的顶层。,导出声明只能在模块的顶层使用。,导出声明只能在命名空间或模块的顶层使用。,导出声明不能有修饰符。,启用“verbatimModuleSyntax”，但“{0}”解析为仅类型声明时，“export =”声明必须引用实际值。,启用“verbatimModuleSyntax”时，“export =”声明必须引用值，但“{0}”仅引用类型。,启用“verbatimModuleSyntax”，但“{0}”解析为仅类型声明时，“export default”必须引用实际值。,启用“verbatimModuleSyntax”时，“export default”必须引用一个值，但“{0}”只引用了一个类型。,"无法测试 ""void"" 类型的表达式的真实性。",扩展的 Unicode 转义值必须介于(含) 0x0 和 0x10FFFF 之间。,标识符或关键字不能紧跟在数字字面量之后。,不能在环境上下文中声明实现。,"导入别名不能引用使用 ""export type"" 导出的声明。","导入别名不能引用使用 ""import type"" 导入的声明。",启用“verbatimModuleSyntax”时，导入别名无法解析为类型或仅类型声明。,导入别名不能使用“导入类型”,导入声明只能在模块的顶层使用。,导入声明只能在命名空间或模块的顶层使用。,导入声明不能有修饰符。,启用“allowImportingTsExtensions”时，导入路径只能以“{0}”扩展名结尾。,索引签名不能包含 rest 参数。,索引签名不能包含尾随逗号。,索引签名必须具有类型批注。,索引签名必须正好具有一个参数。,索引签名参数不能包含问号。,索引签名参数不能具有可访问性修饰符。,索引签名参数不能具有初始化表达式。,索引签名参数必须具有类型批注。,索引签名参数类型不能为文本类型或泛型类型。请考虑改用映射的对象类型。,索引签名参数类型必须是 “string”、“number”、“symbol”或模板文本类型。,实例化表达式不能后跟属性访问。,接口只能扩展具有可选类型参数的标识符/限定名称。,接口只能扩展使用静态已知成员的对象类型或对象类型的交集。,接口无法扩展“{0}”这样的基元类型。它只能扩展其他命名对象类型。,接口函数不能具有初始化表达式。,"迭代器必须具有 ""next()"" 方法。",将 @jsx 杂注与 JSX 片段一起使用时，需要使用 @jsxFrag 杂注。,对象文字不能具有多个具有相同名称的 get/set 访问器。,对象文本不能具有多个名称相同的属性。,对象文字不能包含具有相同名称的属性和访问器。,对象成员无法声明为可选。,对象的“[Symbol.hasInstance]”方法必须返回布尔值，这样它才能在“instanceof”表达式的右侧使用。,可选链不能包含专用标识符。,可选元素不能跟在 rest 元素之后。,"此容器隐藏了 ""this"" 的外部值。",重载签名无法声明为生成器。,乘方表达式的左侧不允许存在具有“{0}”运算符的一元表达式。请考虑用括号将表达式括起。,使用 JSDoc 中的类型批注所有内容,注释命名空间中属性 expando 函数的类型,通过 JSDoc 类型批注,这里是其他导出默认值。,任何可能匹配多个字符的 Unicode 属性仅在设置 Unicode Sets (v) 标志时可用。,可能匹配多个字符的任何内容在被否定的字符类中无效。,是否缺少分号?,应为参数表达式。,“{0}”选项的参数必须为 {1}。,动态导入的参数不能是扩展元素。,类型“{0}”的参数不能赋给类型“{1}”的参数。,类型为“{0}”的参数不能分配给类型为“{1}”且 “exactOptionalPropertyTypes: true” 的参数。请考虑将 “undefined” 添加到目标属性的类型。,未提供 rest 形参“{0}”的实参。,应为数组元素析构模式。,无法使用 --isolatedDeclarations 推断具有分布元素的数组。,断言要求使用显式类型注释声明调用目标中的每个名称。,断言要求调用目标为标识符或限定名。,--isolatedDeclarations 不支持将属性分配给不声明它们的函数。为分配给此函数的属性添加显式声明。,"应为 ""*/""。",至少一个访问器必须具有带有 --isolatedDeclarations 的显式类型注释。,全局范围的扩大仅可直接嵌套在外部模块中或环境模块声明中。,"全局范围的扩大应具有 ""declare"" 修饰符，除非它们显示在已有的环境上下文中。",项目“{0}”中启用了键入内容的自动发现。使用缓存位置“{2}”运行模块“{1}”的额外解决传递。,生成选项,向后兼容性,基类表达式无法引用类类型参数。,"基构造函数返回类型 ""{0}"" 不是具有静态已知成员的对象类型或对象类型的交集。",所有的基构造函数必须具有相同的返回类型。,用于解析非绝对模块名的基目录。,目标低于 ES2020 时，BigInt 字面量不可用。,需要二进制数字。,绑定元素“{0}”隐式具有“{1}”类型。,绑定元素不能直接使用 --isolatedDeclarations 导出。,声明之前已使用的块范围变量“{0}”。,在工作目录中生成复合项目。,生成所有项目，包括那些似乎是最新的项目。,生成一个或多个项目及其依赖项(如果已过期),"生成选项 ""{0}"" 需要类型 {1} 的值。",正在生成项目“{0}”...,内置迭代器实例化时，“TReturn”类型为“undefined”而不是“any”。,命令行标记,常见命令,常见编译器选项,调用修饰器表达式,"调用签名返回类型 ""{0}"" 和 ""{1}"" 不兼容。","缺少返回类型批注的调用签名隐式具有返回类型 ""any""。","没有参数的调用签名具有不兼容的返回类型 ""{0}"" 和 ""{1}""。",调用目标不包含任何签名。,仅可转换逻辑 AND 访问链,只能转换已命名的导出,只能转换带修饰符的属性,只能转换字符串串联和字符串文本,"无法访问“{0}.{1}”，因为“{0}”是类型，不是命名空间。是否要使用“{0}[""{1}""]”检索“{0}”中“{1}”属性的类型?",启用“{1}”时，无法从没有限定的其他文件访问“{0}”。请改用“{2}”。,启用“{0}”时，无法访问环境常量枚举。,不可将“{0}”构造函数类型分配给“{1}”构造函数类型。,无法将抽象构造函数类型分配给非抽象构造函数类型。,无法为“{0}”赋值，因为它是类。,"无法分配到 ""{0}"" ，因为它是常数。",无法为“{0}”赋值，因为它是函数。,无法为“{0}”赋值，因为它是命名空间。,无法为“{0}”赋值，因为它是只读属性。,无法为“{0}”赋值，因为它是枚举。,无法为“{0}”赋值，因为它是导入。,无法为“{0}”赋值，因为它不是变量。,无法赋值给私有方法“{0}”。私有方法不可写。,无法扩大模块“{0}”，因为它解析为非模块实体。,无法扩充具有值导出的模块“{0}”，因为它解析为一个非模块的实体。,"无法使用选项“{0}”来编译模块，除非 ""--module"" 标记为 ""amd"" 或 ""system""。",无法创建抽象类的实例。,"无法将迭代委托到值，因为其迭代器的 ""next"" 方法需要类型 ""{1}""，但包含它的生成器将始终发送 ""{0}""。",无法导出“{0}”。仅可从模块中导出本地声明。,无法扩展类“{0}”。类构造函数标记为私有。,"无法扩展接口“{0}”。您是否想使用 ""implements""?",无法在当前目录找到 tsconfig.json 文件: {0}。,无法在指定目录找到 tsconfig.json 文件:“{0}”。,找不到全局类型“{0}”。,找不到全局值“{0}”。,找不到“{0}”的库定义。,找不到“{0}”的库定义。你是指“{1}”?,"找不到模块“{0}”。请考虑使用 ""--resolveJsonModule"" 导入带 "".json"" 扩展的模块。","找不到模块“{0}”。你的意思是要将 ""moduleResolution"" 选项设置为 ""nodenext""，还是要将别名添加到 ""paths"" 选项中?",找不到模块“{0}”或其相应的类型声明。,找不到名称“{0}”。,找不到名称“{0}”。你是否指的是“{1}”?,找不到名称“{0}”。你的意思是实例成员“this.{0}”?,找不到名称“{0}”。你的意思是静态成员“{1}.{0}”?,找不到名称“{0}”。你是否要在异步函数中写入此内容?,找不到名称“{0}”。是否需要更改目标库? 请尝试将 “lib” 编译器选项更改为“{1}”或更高版本。,找不到名称“{0}”。是否需要更改目标库? 请尝试更改 “lib” 编译器选项以包括 “dom”。,找不到名称“{0}”。你是否需要安装 Bun 的类型定义?请尝试运行 `npm i --save-dev @types/bun`。,"找不到名称“{0}”。你是否需要安装 Bun 的类型定义?请尝试运行 `npm i --save-dev @types/bun`，然后将 ""bun"" 添加到 tsconfig 的 types 字段。","找不到名称 ""{0}""。是否需要安装测试运行器的类型定义? 请尝试使用 `npm i --save-dev @types/jest` 或 `npm i --save-dev @types/mocha`。",找不到名称“{0}”。是否需要安装测试运行器的类型定义? 请尝试使用 `npm i --save-dev @types/jest` 或 `npm i --save-dev @types/mocha`，然后将 “jest” 或 “mocha” 添加到 tsconfig 中的类型字段。。,"找不到名称 ""{0}""。是否需要安装 jQuery 的类型定义? 请尝试使用 `npm i --save-dev @types/jquery`。",找不到名称“{0}”。是否需要安装 jQuery 的类型定义? 请尝试使用 `npm i --save-dev @types/jquery`，然后将 “jquery” 添加到 teconfig 中的类型字段。,找不到名称“{0}”。是否需要安装 Node.js 的类型定义? 请尝试运行 `npm i --save-dev @types/node`。,"找不到名称“{0}”。是否需要安装 Node.js 的类型定义? 请尝试运行 `npm i --save-dev @types/node`，然后将 ""node"" 添加到 tsconfig 的 types 字段。",找不到命名空间“{0}”。,找不到命名空间“{0}”。你是否指的是“{1}”?,找不到参数“{0}”。,找不到输入文件的公共子目录路径。,找不到“{0}”的类型定义文件。,无法导入类型声明文件。请考虑导入“{0}”，而不是“{1}”。,无法在块范围声明“{1}”所在的范围内初始化外部范围变量“{0}”。,"不能调用可能是 ""null"" 的对象。","不能调用可能是 ""null"" 或“未定义”的对象。",不能调用可能是“未定义”的对象。,"无法迭代值，因为其迭代器的 ""next"" 方法需要类型 ""{1}""，但数组析构将始终发送 ""{0}""。","无法迭代值，因为其迭代器的 ""next"" 方法需要类型 ""{1}""，但数组扩张将始终发送 ""{0}""。","无法迭代值，因为其迭代器的 ""next"" 方法需要类型 ""{1}""，但 for-of 将始终发送 ""{0}""。",无法将语句移动到所选文件,无法移动到文件，所选文件无效,无法读取文件“{0}”。,无法读取文件“{0}”: {1}。,无法重新声明块范围变量“{0}”。,无法重新声明导出的变量“{0}”。,无法在 catch 子句中重新声明标识符“{0}”。,无法在类型注释中启动函数调用。,"无法使用 JSX，除非提供了 ""--jsx"" 标志。",启用“{0}”时，不能对类型或仅类型命名空间使用“export import”。,"当 ""--module"" 为 ""none"" 时无法使用导入、导出或模块扩大。",不能将命名空间“{0}”用作类型。,不能将命名空间“{0}”用作值。,无法在修饰类静态属性初始化表达式中使用 “this”。,"无法写入文件 ""{0}""，因为它将覆盖由引用的项目 ""{1}"" 生成的 "".tsbuildinfo"" 文件",无法写入文件“{0}”，因为它会被多个输入文件覆盖。,无法写入文件“{0}”，因为它会覆盖输入文件。,Catch 子句变量不能有初始化表达式。,"Catch 子句变量类型注释必须为 ""any"" 或 ""unknown"" (若已指定)。",将“{0}”更改为“{1}”,"将所有扩展接口都更改为 ""implements""",将所有 JSDoc 样式类型都更改为 TypeScript,"将所有 JSDoc 样式类型都更改为 TypeScript (并将 ""| undefined"" 添加到可以为 null 的类型)","将 ""extends"" 改为 ""implements""",将拼写更改为“{0}”,检查是否有已声明但未在构造函数中设置的类属性。,检查副作用导入。,检查 “bind”、“call” 和 “apply” 方法的参数是否与原始函数匹配。,检查“{0}”是否是“{1}”-“{2}”的最长匹配前缀。,导入别名“{0}”的循环定义。,解析配置时检测到循环: {0},循环源自此位置的类型。,类“{0}”将“{1}”定义为实例成员访问器，但扩展类“{2}”将其定义为实例成员函数。,类“{0}”将“{1}”定义为实例成员函数，但扩展类“{2}”将其定义为实例成员访问器。,类“{0}”将“{1}”定义为实例成员属性，但扩展类“{2}”将其定义为实例成员函数。,类“{0}”错误扩展基类“{1}”。,类“{0}”错误实现类“{1}”。你是想扩展“{1}”并将其成员作为子类继承吗?,类“{0}”错误实现接口“{1}”。,类“{0}”用于其声明前。,类构造函数可能不是生成器。,类构造函数可能不是访问器。,类声明无法实现“{0}”的重载列表。,类声明不能有多个 “@augments” 或 “@extends” 标记。,类修饰器不能与静态专用标识符一起使用。请考虑删除实验性修饰器。,父类定义的类字段“{0}”无法通过 super 在子类中访问。,类名不能为“{0}”。,"使用模块 {0} 将目标设置为 ES5 及更高版本时，类名不能为 ""Object""。",类静态侧“{0}”错误扩展基类静态侧“{1}”。,类只能扩展一个类。,"类不能具有名为 ""constructor"" 的字段。",类中包含的代码在 JavaScript 的严格模式下进行计算，该模式不允许以此方式使用“{0}”。有关详细信息，请参阅 https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Strict_mode。,命令行选项,"编译给定了其配置文件路径或带 ""tsconfig.json"" 的文件夹路径的项目。",编译器诊断,无法为编译器选项“{0}”提供空字符串。,编译器选项“{0}”需要参数。,编译器选项“--{0}”不能与 “--build” 一起使用。,编译器选项“--{0}”只能与 “--build” 一起使用。,值“{1}”的编译器选项“{0}”不稳定。使用夜间 TypeScript 消除此错误。请尝试使用 “npm install -D typescript@next” 进行更新。,编译器选项“{0}”需要类型 {1} 的值。,当发出专用标识符下层时，编译器会预留名称“{0}”。,编译位于指定路径的 TypeScript 项目。,编译当前项目(工作目录中的 tsconfig.json。),使用其他设置编译当前项目。,完成度,复合项目可能不会禁用声明发出。,复合项目不能禁用增量编译。,从输入文件列表计算,计算属性必须是数字或字符串文本、变量或带 --isolatedDeclarations 的虚线表达式。,枚举中不允许计算属性名。,不能使用 --isolatedDeclarations 推断类或对象文本上的计算属性名称。,含字符串值成员的枚举中不允许使用计算值。,连接输出并将其发出到单个文件。,解析导入时除特定于解析程序的默认值外要设置的条件。,此文件中存在冲突。,请考虑向此类添加 “declare” 修饰符。,"构造签名返回类型 ""{0}"" 和 ""{1}"" 不兼容。","缺少返回类型批注的构造签名隐式具有返回类型 ""any""。","不带参数的构造签名具有不兼容的返回类型 ""{0}"" 和 ""{1}""。",缺少构造函数实现。,类“{0}”的构造函数是私有的，仅可在类声明中访问。,类“{0}”的构造函数是受保护的，仅可在类声明中访问。,在联合类型中使用时，构造函数类型标记必须用括号括起来。,在相交类型中使用时，构造函数类型标记必须用括号括起来。,"派生类的构造函数必须包含 ""super"" 调用。","未指定包含文件，并且无法确定根目录，正在跳过在 ""node_modules"" 文件夹中查找。",包含函数不是箭头函数,控制用于检测模块格式 JS 文件的方法。,"类型 ""{0}"" 到类型 ""{1}"" 的转换可能是错误的，因为两种类型不能充分重叠。如果这是有意的，请先将表达式转换为 ""unknown""。",将“{0}”转换为 {0} 中的 {1},将“{0}”转换为映射对象类型,将所有 'const' 转换为 'let',将所有构造函数都转换为类,将所有无效字符转换为 HTML 实体代码,将所有重新导出的类型转换为仅类型导出,"将所有 ""require"" 转换为 ""import""",全部转换为异步函数,全部转换为 BigInt 数字字面量,全部转换为默认导入,将所有类型文本转换为映射类型,将所有 typedef 转换为 TypeScript 类型。,转换箭头函数或函数表达式,"将 ""const"" 转换为 ""let""",将默认导出转换为命名导出,将函数声明“{0}”转换为箭头函数,"将函数表达式 ""{0}"" 转换为箭头函数",将函数转换为 ES2015 类,将无效字符转换为其 HTML 实体代码,将命名导出转换为默认导出,将命名导入转换为默认导入,将命名导入转换为命名空间导入,将命名空间导入转换为命名导入,将重载列表转换为单一签名,将参数转换为析构对象,"将 ""require"" 转换为 ""import""",转换为 ES 模块,转换为 BigInt 数字字面量,转换为异步函数,转换为箭头函数,转换为异步函数,转换为默认导入,转换为指定函数,转换为可选链表达式,转换为模板字符串,转换为仅类型导出,将 typedef 转换为 TypeScript 类型。,区域设置文件 {0} 已损坏。,无法转换为匿名函数,无法转换为箭头函数,无法转换为命名函数,无法确定函数返回类型,找不到包含箭头函数,"无法找到模块“{0}”的声明文件。“{1}”隐式拥有 ""any"" 类型。",找不到可转换的访问表达式,找不到 export 语句,找不到 import 子句,找不到匹配的访问表达式,找不到名称“{0}”。你是否是指“{1}”?,找不到命名空间导入或已命名的导入,找不到要为其生成访问器的属性,找不到要内联的变量。,无法解析具有表达式的路径“{0}”: {1}。,无法写入文件“{0}”: {1}。,为发出的 JavaScript 文件创建源映射文件。,为 d.ts 文件创建源映射。,使用工作目录中的建议设置创建 tsconfig.json。,目录,字符类中不允许使用十进制转义序列和反向引用。,不允许使用带前导零的小数。,该声明扩充了另一文件中的声明。这无法被序列化。,为此文件发出的声明需要保留此导入以进行扩充。--isolatedDeclarations 不支持此功能。,"此文件的声明发出要求使用专用名称 ""{0}""。显式类型注释可能取消阻止声明发出。","此文件的声明发出要求使用模块 ""{1}"" 中的专用名称 ""{0}""。显式类型注释可能取消阻止声明发出。",为此参数发出的声明要求将未定义隐式添加到其类型。--isolatedDeclarations 不支持此功能。,应为声明。,声明名称与内置全局标识符“{0}”冲突。,应为声明或语句。,"应为声明或语句。此 ""="" 遵循语句块，因此如果打算编写重构赋值，则可能需要用括号将整个赋值括起来。",具有明确赋值断言的声明也必须具有类型批注。,具有初始值设定项的声明不能同时具有明确赋值断言。,"声明名为 ""{0}"" 的专用字段。",声明方法“{0}”,"声明私有方法 ""{0}""",声明专用属性“{0}”,声明属性“{0}”,声明静态方法“{0}”,声明静态属性“{0}”,装饰器函数返回类型“{0}”不可分配到类型“{1}”。,装饰器函数返回类型为“{0}”，但预期为“void”或“any”。,此处“导出”之前使用的修饰器。,修饰器在此处无效。,不能向多个同名的 get/set 访问器应用修饰器。,如果修饰器也出现在“导出”之前，则它们可能不会出现在“export”或“export default”之后。,修饰器必须位于属性声明的名称和所有关键字之前。,将 catch 子句变量默认为 “unknown” 而不是 “any”。,模块的默认导出具有或正在使用专用名称“{0}”。,无法使用 --isolatedDeclarations 推断默认导出。,延迟导入中不允许使用默认导入。,默认库,"目标 ""{0}"" 的默认库",仅当 ‘--module’ 标志设置为 ‘esnext’ 或 ‘preserve’ 时，才支持延迟导入。,以下标识符的定义与另一个文件中的定义冲突: {0},删除未使用的所有声明,删除所有未使用的导入,删除所有未使用的 “@param” 标记,删除所有项目的输出。,删除未使用的 “@param” 标记“{0}”,"[已弃用] 请改用 ""--jsxFactory""。已 ""react"" JSX 发出设为目标时，请指定要为 createElement 调用的对象","[已弃用] 请改用 ""--outFile""。连接并发出到单个文件的输出","[已弃用] 请改用 ""--skipLibCheck""。请跳过默认库声明文件的类型检查。",弃用的设置。请改用 “outFile”。,"是否忘记使用 ""await""?","你是想使用 ""{0}"" 吗?","你是想将 ""{0}"" 限制为类型 ""new (...args: any[]) => {1}"" 吗?",你是想调用此表达式吗?,"你是想将此函数标记为 ""async"" 吗?","你的意思是使用 "":"" 吗? 当包含对象文字属于解构模式时，""="" 只能跟在属性名称的后面。","你是想将 ""new"" 用于此表达式吗?",应为数字。,目录“{0}”不存在，正在跳过该目录中的所有查找。,目录 '{0}' 不包含 package.json 作用域。导入将无法解析。,禁止在发出的 JavaScript 文件中添加 “use strict” 指令。,禁用检查此文件,禁用发出注释。,禁用在其 JSDoc 注释中包含 “@internal” 的发出声明。,禁用从编译发出文件。,禁用在报告了任何类型检查错误时发出文件。,在生成的代码中禁用擦除 “const enum” 声明。,对无法访问的代码禁用错误报告。,对未使用的标签禁用错误报告。,禁用完整类型检查（仅报告关键分析和发出错误）。,在已编译输出中禁用生成自定义帮助程序函数(如 “__extends”)。,禁用包括任何库文件(包括默认的 lib.d.ts)。,禁止加载引用的项目。,在引用复合项目时禁用首选源文件而不是声明文件。,禁用在创建对象文字期间报告多余属性错误。,禁用将符号链接解析为其实际路径。这会关联到节点中的同一标志。,禁用对 JavaScript 项目的大小限制。,对此项目禁用解决方案搜索。,禁止严格检查函数类型中的通用签名。,禁用针对 JavaScript 项目的类型获取,在错误消息中禁用截断类型。,禁止使用源文件而不是引用项目中的声明文件。,禁用在监视模式下擦除控制台。,禁用通过查看项目中的文件名进行类型获取推理。,禁止 “import”、“require” 或 “<reference>” 扩展 TypeScript 应添加到项目的文件数。,不允许对同一文件采用大小不一致的引用。,请勿将三斜杠引用或导入的模块添加到已编译文件列表中。,不允许不属于 ECMAScript 的运行时构造。,请勿将注释发出到输出。,"请勿对具有 ""@internal"" 注释的代码发出声明。",请勿发出输出。,如果报告了任何错误，请不要发出输出。,"不要在模块输出中发出 ""use strict"" 指令。",请勿清除生成代码中的常数枚举声明。,"请勿在已编译输出中生成自定义帮助程序函数，例如 ""__extends""。",请勿包括默认库文件(lib.d.ts)。,不报告有关不可访问的代码的错误。,不报告有关未使用的标签的错误。,不要解析 symlink 的真实路径。,不要转换或省略任何未标记为仅类型的导入或导出，确保它们是根据“模块”设置以输出文件格式编写的。,请勿删除错误消息。,函数实现重复。,标识符“{0}”重复。,标识符“{0}”重复。编译器在模块的顶层范围中保留名称“{1}”。,标识符“{0}”重复。编译器在包含异步函数的模块的顶层范围中保留名称“{1}”。,标识符“{0}”重复。在静态初始化表达式中中发出 “super” 引用时，编译器保留名称“{1}”。,标识符“{0}”重复。编译器使用“{1}”声明来支持异步函数。,"标识符 ""{0}"" 重复。静态元素和实例元素不能共享相同的专用名称。","标识符 ""arguments"" 重复。编译器使用 ""arguments"" 初始化 rest 参数。","标识符 ""_newTarget"" 重复。编译器使用变量声明 ""_newTarget"" 来捕获 ""new.target"" 元属性引用。","标识符 ""_this"" 重复。编译器使用变量声明 ""_this"" 来捕获 ""this"" 引用。",类型“{0}”的索引签名重复。,标签“{0}”重复。,"重复的属性 ""{0}""。",正则表达式标志重复。,"动态导入的说明符类型必须是 ""string""，但此处类型是 ""{0}""。",仅当将 ‘--module’ 标记设置为 ‘es2020’、‘es2022’、‘esnext’、‘commonjs’、‘amd’、‘system’、‘umd’、‘node16’、‘node18’、'node20' 或 ‘nodenext’ 时，才支持动态导入。,动态导入只能接受模块说明符和可选的特性集作为参数,仅当 ‘--module’ 选项设置为 ‘esnext’、‘node16’、‘node18’、‘node20’、‘nodenext’ 或 ‘preserve’ 时，动态导入才支持第二个参数。,当“module”设置为“preserve”时，CommonJS 模块中不允许使用 ESM 语法。,启用“verbatimModuleSyntax”时，CommonJS 模块中不允许使用 ESM 语法。,“{0}.{1}”的每个声明的值不同，其中应为“{2}”，但给出的是“{3}”。,"联合类型 ""{0}"" 的每个成员都有构造签名，但这些签名都不能互相兼容。","联合类型 ""{0}"" 的每个成员都有签名，但这些签名都不能互相兼容。",编辑器支持,"元素隐式具有 ""any"" 类型，因为类型为 ""{0}"" 的表达式不能用于索引类型 ""{1}""。","元素隐式具有 ""any"" 类型，因为索引表达式的类型不为 ""number""。","元素隐式具有 ""any"" 类型，因为类型“{0}”没有索引签名。","元素隐式具有 ""any"" 类型，因为类型 ""{0}"" 没有索引签名。你是想调用 ""{1}"" 吗?",发出,发出符合 ECMAScript 标准的类字段。,在输出文件的开头发出一个 UTF-8 字节顺序标记(BOM)。,发出包含源映射而非包含单独文件的单个文件。,发出用于调试的编译器运行的 v8 CPU 配置文件。,发出其他 JavaScript 以轻松支持导入 CommonJS 模块。这将启用 “allowSyntheticDefaultImports” 以实现类型兼容性。,使用 Define 而不是 Set 发出类字段。,为源文件中的修饰声明发出设计类型元数据。,发出更合规但更详细且性能较低的 JavaScript 进行迭代。,"在单个文件内发出源以及源映射；需要设置 ""--inlineSourceMap"" 或 ""--sourceMap""。",启用所有严格类型检查选项。,在 TypeScript 输出中启用颜色和格式设置，以使编译器错误更易于阅读。,启用允许将 TypeScript 项目与项目引用一起使用的约束。,为未在函数中显式返回的代码路径启用错误报告。,对具有隐式 “any” 类型的表达式和声明启用错误报告。,为 switch 语句中的故障案例启用错误报告。,在已检查类型的 JavaScript 文件中启用错误报告。,在未读取局部变量时启用错误报告。,在 “this” 的类型为 “any” 时启用错误报告。,启用对旧实验性修饰器的实验性支持。,启用导入具有任何扩展名的文件，前提是存在声明文件。,启用导入 .json 文件。,启用 lib 替换。,启用项目编译,"对函数启用严格的 ""bind""、""call"" 和 ""apply"" 方法。",对函数类型启用严格检查。,启用类中属性初始化的严格检查。,启用严格的 NULL 检查。,"在配置文件中启用 ""experimentalDecorators"" 选项","在配置文件中启用 ""--jsx"" 标志",启用名称解析过程的跟踪。,启用详细日志记录。,"通过为所有导入创建命名空间对象来启用 CommonJS 和 ES 模块之间的发出互操作性。表示 ""allowSyntheticDefaultImports""。",对 ES7 修饰器启用实验支持。,对发出修饰器的类型元数据启用实验支持。,对使用索引类型声明的键强制使用索引访问器。,确保使用替代修饰符标记派生类中的替代成员。,确保导入中的大小写正确。,确保可以安全地转译每个文件，而无需依赖其他导入。,请确保始终发出 “se strict”。,正在输入条件导出。,"隐式类型库 ""{0}"" 的入口点","隐式类型库 ""{0}"" 的入口点，具有 packageId ""{1}""","在 compilerOptions 中指定的类型库 ""{0}"" 的入口点","在 compilerOptions 中指定的类型库 ""{0}"" 的入口点，具有 packageId ""{1}""",枚举“{0}”用于其声明前。,枚举声明只能与命名空间或其他枚举声明合并。,枚举声明必须全为常数或非常数。,应为枚举成员。,启用“isolatedModules”时，非文本数值成员之后的枚举成员必须具有初始值设定项。,枚举成员初始值设定项必须可计算，而不引用具有 --isolatedDeclarations 的外部符号。,枚举成员必须具有初始化表达式。,枚举名不能为“{0}”。,环境设置,错误文件,不允许转义序列“{0}”。,示例: {0},比较类型“{0}”和“{1}”的复杂性过高。,与类型“{0}”和“{1}”相比，堆栈深度过高。,正在退出条件导出。,"应为 {0}-{1} 类型参数；请为这些参数添加 ""@extends"" 标记。",应有 {0} 个参数，但获得 {1} 个。,"应为 {0} 个参数，但得到的却是 {1} 个。你是否忘了将类型参数中的 ""void"" 包含到 ""Promise""?",应有 {0} 个类型参数，但获得 {1} 个。,"应为 {0} 类型参数；请为这些参数添加 ""@extends"" 标记。",应为 1 个参数，但得到 0。“new Promise()” 需要 JSDoc 提示才能生成可在没有参数的情况下调用的 “resolve”。,应为 Unicode 属性名称。,应为 Unicode 属性名称或值。,应为 Unicode 属性值。,应为捕获组名称。,应为类集操作数。,应有至少 {0} 个参数，但获得 {1} 个。,“{0}”预期的相应 JSX 结束标记。,预期的 JSX 片段的相应结束标记。,"属性初始化表达式应有 ""=""。","""package.json"" 中 ""{0}"" 字段的类型应为 ""{1}""，但实际为 ""{2}"" 。",显示指定了模块解析类型:“{0}”。,"除非 ""target"" 选项设置为 ""es2016"" 或更高版本，否则不能对 ""bigint"" 值执行求幂运算。",从模块“{1}”导出“{0}”,导出所有引用的局部变量,"面向 ECMAScript 模块时，不能使用导出分配。请考虑改用 ""export default"" 或另一种模块格式。","当 ""--module"" 标志是 ""system"" 时不支持导出分配。",导出声明与“{0}”的导出声明冲突。,命名空间中不允许有导出声明。,路径 '{1}' 处的 package.json 作用域中不存在导出说明符 '{0}'。,导出的类型别名“{0}”已经或正在使用专用名称“{1}”。,导出的类型别名“{0}”具有或正在使用模块“{2}”中的专用名称“{1}”。,导出的变量“{0}”具有或正在使用外部模块“{2}”中的名称“{1}”，但不能为其命名。,导出的变量“{0}”具有或正在使用私有模块“{2}”中的名称“{1}”。,导出的变量“{0}”具有或正在使用专用名称“{1}”。,模块扩大中不允许导出和导出分配。,应为表达式。,表达式必须括在括号中才能用作修饰器。,应为表达式或逗号。,表达式生成的元组类型太大，无法表示。,表达式生成的联合类型过于复杂，无法表示。,"表达式解析为 ""_super""，编译器使用 ""_super"" 获取基类引用。","表达式解析为编辑器用于捕获 ""new.target"" 元属性引用的变量声明 ""_newTarget""。","表达式解析为编译器用于捕获 ""this"" 引用的变量声明 ""_this""。",无法使用 --isolatedDeclarations 推断表达式类型。,Extends 子句不能包含具有 --isolatedDeclarations 的表达式。,推断类型“{0}”的 Extends 子句具有或正在使用专用名称“{1}”。,将基类提取到变量,将绑定表达式提取到变量,提取常数,将默认导出提取到变量,提取函数,提取到 {1} 中的 {0},提取到 {1} 范围中的 {0},提取到封闭范围中的 {0},提取到接口,提取到类型别名,提取到类型引用,提取到变量并替换为“{0} 为 typeof {0}”,提取类型,文件,文件或目录,找不到 peerDependency“{0}”。,无法在条件“{0}”下解析。,switch 语句中的 Fallthrough 情况。,文件“{0}”不存在。,根据前面缓存的查找，文件“{0}”不存在。,根据前面缓存的查找，文件“{0}”存在。,文件“{0}”存在 - 将其用作名称解析结果。,文件“{0}”具有不受支持的扩展名。仅支持 {1} 扩展名。,"文件 ""{0}"" 是 JavaScript 文件。你是想启用 ""allowJs"" 选项吗?",文件“{0}”不是模块。,"文件 ""{0}"" 不在项目 ""{1}"" 的文件列表中。项目必须列出所有文件，或使用 ""include"" 模式。","文件“{0}”不在 ""rootDir""“{1}”下。""rootDir"" 应包含所有源文件。",找不到文件“{0}”。,文件布局,文件管理,文件似乎是二进制文件。,检测到文件更改。正在启动增量编译...,文件是 CommonJS 模块，因为“{0}”没有字段 “type”,文件是 CommonJS 模块，因为“{0}”具有值不是 “module” 的字段 “type”,文件是 CommonJS 模块，因为找不到 “package.json”,文件是 ECMAScript 模块，因为“{0}”具有值为 “module” 的字段 “type”,文件是 CommonJS 模块; 它可能会转换为 ES 模块。,文件是此处指定的目标的默认库。,文件是此处指定的类型库的入口点。,在此处通过导入包含了文件。,在此处通过库引用包含了文件。,在此处通过引用包含了文件。,在此处通过类型库引用包含了文件。,文件是此处指定的库。,通过此处指定的“文件”列表匹配了文件。,通过在此处指定包含模式匹配了文件。,从此处指定的引用项目输出文件。,"文件是项目引用源 ""{0}"" 的输出",文件源自此处指定的引用项目。,文件名“{0}”仅在大小写方面与包含的文件名“{1}”不同。,文件名“{0}”具有“{1}”扩展 - 改为查找“{2}”。,文件名“{0}”的扩展名为“{1}”，请去除它。,"文件重定向到文件 ""{0}""","文件规范不能包含出现在递归目录通配符(""*""): “{0}”后的父目录("".."")。","文件规范不能以递归目录通配符结尾(""**""):“{0}”。",从 “include” 选项筛选结果。,修复检测到的所有拼写错误,"修复可能缺少 ""await"" 的所有表达式",修复所有 implicit-'this' 错误,修复所有错误的异步函数返回类型,使用仅类型导入修复所有问题,对于 nodejs:,找到 {0} 个错误。,找到 {0} 个错误。注意文件更改。,在 {1} 个文件中找到 {0} 个错误。,在同一文件中找到 {0} 个错误，起始位置为: {1},找到 1 个错误。,找到 1 个错误。注意文件更改。,在 {0} 中找到 1 个错误,"在“{0}”处找到了 ""package.json""。",找到具有“{1}”版本的 peerDependency“{0}”。,面向“ES5”时，在严格模式下，块内不允许函数声明。,面向“ES5”时，在严格模式下，块内不允许函数声明。类定义自动处于严格模式。,面向“ES5”时，在严格模式下，块内不允许函数声明。模块自动处于严格模式。,缺少返回类型批注的函数表达式隐式具有“{0}”返回类型。,函数实现缺失或未立即出现在声明之后。,函数实现名称必须为“{0}”。,"由于函数不具有返回类型批注并且在它的一个返回表达式中得到直接或间接引用，因此它隐式具有返回类型 ""any""。","函数缺少结束 return 语句，返回类型不包括 ""undefined""。",函数必须具有带有 --isolatedDeclarations 的显式返回类型注释。,未实现函数。,函数重载必须为静态。,函数重载不能为静态。,在联合类型中使用时，函数类型标记必须用括号括起来。,在相交类型中使用时，函数类型标记必须用括号括起来。,"缺少返回类型注释的函数类型隐式具有 ""{0}"" 返回类型。",具有正文的函数只能与环境类合并。,从项目的 TypeScript 和 JavaScript 文件生成 .d.ts 文件。,"生成 ""get"" 和 ""set"" 访问器","为所有重写属性生成 ""get"" 和 ""set"" 访问器",生成 CPU 配置文件。,"为每个相应的 "".d.ts"" 文件生成源映射。",生成事件跟踪和类型列表。,"生成相应的 "".d.ts"" 文件。","生成相应的 "".map"" 文件。",生成器隐式具有 yield 类型 ‘{0}’。请考虑提供一个返回类型注释。,不允许在环境上下文中使用生成器。,泛型类型“{0}”需要 {1} 个类型参数。,泛型类型“{0}”需要介于 {1} 和 {2} 类型参数之间。,全局模块导出仅可出现在顶层级别中。,全局模块导出仅可出现声明文件中。,全局模块导出仅可出现模块文件中。,全局类型“{0}”必须为类或接口类型。,全局类型“{0}”必须具有 {1} 个类型参数。,"在 ""--incremental"" 和 ""--watch"" 中有重新编译，假定文件中的更改只会影响直接依赖它的文件。",在使用 “incremental” 和 “watch” 模式的项目中具有重新编译会假定文件中的更改将仅直接影响依赖于它的文件。,应为十六进制数字。,应为标识符。“{0}”是模块顶层的预留字。,应为标识符。“{0}”在严格模式下是保留字。,应为标识符。“{0}”在严格模式下是保留字。类定义自动处于严格模式。,应为标识符。“{0}”是严格模式下的保留字。模块自动处于严格模式。,"应为标识符。""{0}"" 是保留字，不能在此处使用。",应为标识符。,"应为标识符。转换 ECMAScript 模块时，""__esModule"" 保留为导出标记。",应为标识符或字符串字面量。,应为标识符、字符串文本或数字文本。,如果“{0}”包实际上公开此模块，请考虑发送拉取请求以修正“https://github.com/DefinitelyTyped/DefinitelyTyped/tree/master/types/{1}”,如果“{0}”包实际公开了此模块，请尝试添加包含 `declare module‘{1}';` 的新声明(.d.ts)文件,忽略此错误信息,忽略 tsconfig.json，使用默认编译器选项编译指定文件。,实现继承的所有抽象类,实现未实现的所有接口,实现已继承的抽象类,实现接口“{0}”,导出的类“{0}”的 Implements 子句具有或正在使用专用名称“{1}”。,"""symbol"" 到 ""string"" 的隐式转换将在运行时失败。请考虑在 ""String(...)"" 中包装此表达式。",导入“{0}”与此文件中使用的全局值冲突，因此在启用“isolatedModules”时必须使用仅类型导入进行声明。,导入“{0}”与本地值冲突，因此在启用“isolatedModules”时必须使用仅类型导入进行声明。,从“{1}”导入“{0}”,导入断言值必须为字符串字面量表达式。,不允许在编译到 commonJS“require”调用的语句导入断言。,仅当 ‘--module’ 选项设置为 ‘esnext’、‘node18’、‘node20’、‘nodenext’ 或 ‘preserve’ 时，才支持导入断言。,导入断言不能用于仅类型导入或导出。,导入断言已被导入属性替换。使用 “with” 而不是 “assert”。,"面向 ECMAScript 模块时，不能使用导入分配。请考虑改用 ""import * as ns from ""mod""""、""import {a} from ""mod""""、""import d from ""mod"""" 或另一种模块格式。",导入属性值必须为字符串字面量表达式。,不允许在编译到 commonJS“require” 调用的语句导入属性。,仅当 ‘--module’ 选项设置为 ‘esnext’、‘node18’、‘node20’、‘nodenext’ 或 ‘preserve’ 时，才支持导入属性。,导入属性不能用于仅类型导入或导出。,导入声明“{0}”使用的是专用名称“{1}”。,导入声明与“{0}”的局部声明冲突。,命名空间中的导入声明不能引用模块。,"从 ""tslib"" 导入发出帮助程序。",导入可能会转换为默认导入。,导入名称不能为“{0}”。,环境模块声明中的导入或导出声明不能通过相对模块名引用模块。,路径 '{1}' 处的 package.json 作用域中不存在导入说明符 '{0}'。,"通过 {0} 从文件 ""{1}"" 导入","通过 {0} 从文件 ""{1}"" 导入，以按照 compilerOptions 中指定的配置导入 ""importHelpers""","通过 {0} 从文件 ""{1}"" 导入，以导入 ""jsx"" 和 ""jsxs"" 工厂函数","通过 {0} 从具有 packageId ""{2}"" 的文件 ""{1}"" 导入","通过 {0} 从具有 packageId ""{2}"" 的文件 ""{1}"" 导入，以按照 compilerOptions 中指定的方式导入 ""importHelpers""","通过 {0} 从具有 packageId ""{2}"" 的文件 ""{1}"" 导入，以导入 ""jsx"" 和 ""jsxs"" 工厂函数",当 ‘module’ 设置为 ‘{0}’ 时，将 JSON 文件导入 ECMAScript 模块需要 ‘type: “json”’ 导入属性。,模块扩大中不允许导入。请考虑将它们移动到封闭的外部模块。,在环境枚举声明中，成员初始化表达式必须是常数表达式。,在包含多个声明的枚举中，只有一个声明可以省略其第一个枚举元素的初始化表达式。,包含文件列表。这不支持 glob 模式，与 “include” 不同。,"包括通过 "".json"" 扩展导入的模块",在发出的 JavaScript 内的源映射中包含源代码。,在发出的 JavaScript 中包括源映射文件。,包含由“{0}”引用的类型的导入,包括 --watch，-w 将开始监视当前项目的文件更改。设置后，可以使用以下内容配置监视模式:,限定符不完整。应为数字。,类型“{1}”中缺少类型“{0}”的索引签名。,类型“{0}”中的索引签名仅允许读取。,合并声明“{0}”中的单独声明必须全为导出或全为局部声明。,从使用情况推导所有类型,推断函数返回类型,根据使用情况推断参数类型,"从用法中推断出 ""{0}"" 的 ""this"" 类型",根据使用情况推断“{0}”的类型,--isolatedDeclarations 不支持从类表达式推理。,初始化构造函数中的属性“{0}”,初始化静态属性“{0}”,属性“{0}”的初始化表达式,实例成员变量“{0}”的初始化表达式不能引用构造函数中声明的标识符“{1}”。,不允许在环境上下文中使用初始化表达式。,初始化 TypeScript 项目并创建 tsconfig.json 文件。,内联变量,从文件插入命令行选项和文件。,安装“{0}”,安装缺少的所有类型包,接口“{0}”不能同时扩展类型“{1}”和“{2}”。,接口“{0}”错误扩展接口“{1}”。,"接口声明不能有 ""implements"" 子句。",必须为接口指定名称。,接口名称不能为“{0}”。,互操作约束,"将可选属性类型解释为已写，而不是添加 ""undefined""。",无效的字符。,无效的导入说明符 '{0}' 没有可行的解决方法。,扩大中的模块名称无效。模块“{0}”解析到位于“{1}”处的非类型化模块，其无法扩大。,扩大中的模块名无效，找不到模块“{0}”。,新表达式中的可选链无效。是否要调用“{0}()”?,"""reference"" 指令语法无效。",修饰器中的语法无效。,“{0}”的使用无效。它不能在类静态块内使用。,“{0}”的使用无效。模块自动处于严格模式。,严格模式下“{0}”的使用无效。,“--ignoreDeprecations”的值无效。,"""jsxFactory"" 的值无效。“{0}”不是有效的标识符或限定名称。","""jsxFragmentFactory"" 的值无效。“{0}”不是有效的标识符或限定名称。","""--reactNamespace"" 的值无效。“{0}”不是有效的标识符。",很可能缺少了分隔这两个模板表达式的逗号。它们构成了无法调用的带标记的模板表达式。,"其元素类型 ""{0}"" 不是有效的 JSX 元素。","其实例类型 ""{0}"" 不是有效的 JSX 元素。","其返回类型 ""{0}"" 不是有效的 JSX 元素。",其类型“{0}”不是有效的 JSX 元素类型。,"JSDoc ""@{0} {1}"" 不匹配 ""extends {2}"" 子句。","JSDoc ""@{0}"" 未附加到类。","JSDoc ""..."" 可能仅出现在签名的最后一个参数中。","JSDoc ""@param"" 标记具有名称 ""{0}""，但不存在具有该名称的参数。","JSDoc ""@param"" 标记的名称为“{0}”，但该名称没有参数。如果它为数组类型，将匹配 ""arguments""。",JSDoc typedef 可以转换为 TypeScript 类型。,"JSDoc ""@typedef"" 标记应具有类型注释，或其后跟有 ""@property"" 或 ""@member"" 标记。",JSDoc typedef 可以转换为 TypeScript 类型。,JSDoc 类型只能在文档注释内部使用。,JSDoc 类型可能会移到 TypeScript 类型。,只能为 JSX 属性分配非空“表达式”。,JSX 元素“{0}”没有相应的结束标记。,JSX 元素类不支持特性，因为它不具有“{0}”属性。,"JSX 元素隐式具有类型 ""any""，因为不存在接口 ""JSX.{0}""。","JSX 元素隐式具有类型 ""any""，因为不存在全局类型 ""JSX.Element""。",JSX 元素类型“{0}”不具有任何构造签名或调用签名。,JSX 元素不能具有多个名称相同的特性。,JSX 表达式不能使用逗号运算符。你是想写入数组吗?,JSX 表达式必须具有一个父元素。,JSX 片段没有相应的结束标记。,JSX 属性访问表达式不能包含 JSX 命名空间名称,JSX 扩展子属性必须为数组类型。,JavaScript 支持,跳转目标不能跨越函数边界。,种类,关键字不能包含转义字符。,位置,语言和环境,逗号运算符的左侧未使用，没有任何副作用。,"CompilerOptions 中指定了库 ""{0}""","通过 ""{0}"" 从文件 ""{1}"" 引用了库",不允许在此处换行。,箭头前不允许有行终止符。,解析模块时要搜索的文件名后缀列表。,包含类型定义来源的文件夹列表。,根文件夹列表，其组合内容表示在运行时的项目结构。,正在从根目录“{1}”加载“{0}”，候选位置“{2}”。,"正在从 ""node_modules"" 文件夹加载模块“{0}”，目标文件类型: {1}。",正在将模块作为文件/文件夹进行加载，候选模块位置“{0}”，目标文件类型: {1}。,区域设置必须采用 <语言> 或 <语言>-<区域> 形式。例如“{0}”或“{1}”。,在 “moduleResolution” 进程期间使用的日志路径。,“{0}”的最长匹配前缀为“{1}”。,"正在 ""node_modules"" 文件夹中查找，初始位置为“{0}”。","让所有 ""super()"" 调用成为构造函数中的第一个语句",使 keyof 仅返回字符串，而不是字符串、数字或符号。旧版选项。,"在构造函数中，使 ""super()"" 调用第一个语句","映射的对象类型隐式地含有 ""any"" 模板类型。",将数组文字标记为常量,匹配的“{0}”条件“{1}”。,默认情况下匹配包括模式 “**/*”,"通过在 ""{1}"" 中的包含模式 ""{0}"" 匹配",成员“{0}”隐式包含类型“{1}”。,"成员 ""{0}"" 隐式具有 ""{1}"" 类型，但可以从用法中推断出更好的类型。",遇到合并冲突标记。,合并声明“{0}”不能包含默认导出声明。请考虑改为添加一个独立的“导出默认 {0}”声明。,元属性“{0}”只能在函数声明、函数表达式或构造函数的主体中使用。,方法“{0}”不能具有实现，因为它标记为抽象。,导出接口的方法“{0}”具有或正在使用私有模块“{2}”中的名称“{1}”。,导出接口的方法“{0}”具有或正在使用专用名称“{1}”。,方法必须具有带有 --isolatedDeclarations 的显式返回类型注释。,方法未实现。,修饰符不能出现在此处。,"模块 ""{0}"" 只能在使用 ""{1}"" 标志时进行默认导入",无法使用此构造导入模块“{0}”。说明符仅解析为 ES 模块，后者不能使用“require”进行导入。请改用 ECMAScript 导入。,"模块 ""{0}"" 在本地声明 ""{1}""，但它被导出为 ""{2}""。","模块 ""{0}"" 在本地声明 ""{1}""，但未导出它。","模块 ""{0}"" 不引用类型，但在此处用作类型。你是想使用 ""typeof import('{0}')"" 吗?",模块“{0}”不引用值，但在此处用作值。,模块 {0} 已导出一个名为“{1}”的成员。请考虑重新显式导出以解决歧义。,模块“{0}”没有默认导出。,"模块 ""{0}"" 没有默认导出。你是想改为使用 ""import { {1} } from {0}"" 吗?",模块“{0}”没有导出的成员“{1}”。,"模块 ""{0}"" 没有导出的成员 ""{1}""。你是想改用 ""import {1} from {0}"" 吗?",模块“{0}”被具有相同名称的局部声明隐藏。,"模块“{0}”使用 ""export ="" 且无法与 ""export *"" 一起使用。",模块“{0}”解析为文件“{1}”中本地声明的环境模块。,模块“{0}”已解析为“{1}”，但未设置“--allowArbitraryExtensions”。,"模块“{0}”已解析为“{1}”，但尚未设置 ""--jsx""。","模块 ""{0}"" 已解析为 ""{1}""，但未使用 ""--resolveJsonModule""。","模块声明名称只能使用 ' 或 "" 引用字符串。",模块名“{0}”，匹配的模式“{1}”。,======== 未解析模块名“{0}”。========,======== 模块名“{0}”已成功解析为“{1}”。========,"======== 模块名 ""{0}"" 已成功解析为 ""{1}""，包 ID 为 ""{2}""。========",未指定模块解析类型，正在使用“{0}”。,"使用 ""rootDirs"" 的模块解析失败。",模块,将已标记的元组元素修饰符移至标签,将默认导出中的表达式移动到变量，并向其添加类型注释。,移动到新的文件,移动到文件,不允许使用多个连续的数字分隔符。,不允许存在多个构造函数实现。,换行符,名称无效,命名捕获组仅在面向“ES2018”或更高版本时可用。,名称相同的命名捕获组必须彼此排斥。,延迟导入中不允许使用命名导入。,当 ‘module’ 设置为 ‘{0}’ 时，不允许从 JSON 文件到 ECMAScript 模块中的命名导入。,“{1}”和“{2}”类型的命名属性“{0}”不完全相同。,命名空间“{0}”没有已导出的成员“{1}”。,必须为命名空间指定名称。,命名空间名称不能为“{0}”。,启用“{0}”时，全局脚本文件中不允许使用命名空间。如果此文件不是全局脚本，请将“moduleDetection”设置为“force”或添加空的“export {}”语句。,修饰器和修饰符都不能应用于“this”参数。,没有任何基构造函数具有指定数量的类型参数。,"不可调用 ""{0}"" 类型的任何组成部分。","不可构造 ""{0}"" 类型的任何组成部分。","在类型 ""{1}"" 上找不到具有类型为 ""{0}"" 的参数的索引签名。","在配置文件“{0}”中找不到任何输入。指定的 ""include"" 路径为“{1}”，""exclude"" 路径为“{2}”。",不再受支持。在早期版本中，手动设置用于读取文件的文本编码。,没有需要 {0} 参数的重载，但存在需要 {1} 或 {2} 参数的重载。,没有需要 {0} 类型参数的重载，但存在需要 {1} 或 {2} 类型参数的重载。,没有与此调用匹配的重载。,无法从该类型节点中提取任何类型,"速记属性 ""{0}"" 的范围内不存在任何值。请声明一个值或提供一个初始值设定项。",非抽象类“{0}”不会实现继承自“{2}”类的抽象成员 {1}。,非抽象类“{0}”缺少“{1}”的以下成员的实现: {2}。,非抽象类“{0}”缺少“{1}”的以下成员的实现: {2} 和 {3} 等。,非抽象类表达式不会实现继承自“{1}”类的抽象成员“{0}”。,非抽象类表达式缺少“{0}”的以下成员的实现: {1}。,非抽象类表达式缺少“{0}”的以下成员的实现: {1} 和 {2} 等。,非 null 断言只能在 TypeScript 文件中使用。,"未设置 ""baseUrl"" 时，不允许使用非相对路径。是否忘记了前导 ""./""?",此处声明了非简单参数。,并非所有代码路径都返回值。,"""{0}"" 类型的部分要素不可调用。","""{0}"" 类型的部分要素不可构造。",数字在限定符中顺序不对。,绝对值大于或等于 2^53 的数值文本过大，无法用整数准确表示。,此处不允许使用数字分隔符。,"对象的类型为 ""unknown""。","对象可能为 ""null""。","对象可能为 ""null"" 或“未定义”。",对象可能为“未定义”。,对象字面量只能指定已知属性，并且“{0}”不在类型“{1}”中。,对象字面量只能指定已知的属性，但“{0}”中不存在类型“{1}”。是否要写入 {2}?,对象字面量的属性“{0}”隐式含有“{1}”类型。,无法使用 --isolatedDeclarations 推断包含速记属性的对象。,无法使用 --isolatedDeclarations 推断包含传播赋值的对象。,需要八进制数字。,字符类中不允许使用八进制转义序列和反向引用。如果这是转义序列，请改用语法“{0}”。,不允许使用八进制转义序列。请使用语法“{0}”。,不允许使用八进制文字。请使用语法“{0}”。,“{0}.{1}”的一个值是字符串“{2}”，另一个值被假定为未知数值。,"""for...in"" 语句中只允许单个变量声明。","""for...of"" 语句中只允许单个变量声明。","使用 ""new"" 关键字只能调用 void 函数。",仅环境模块可使用带引号的名称。,"--{0} 旁仅支持 ""amd"" 和 ""system"" 模块。",只能使用 --isolatedDeclarations 推断常量数组。,"仅发出 "".d.ts"" 声明文件。 ",仅输出 d.ts 文件，而不输出 JavaScript 文件。,"通过 ""super"" 关键字只能访问基类的公共方法和受保护方法。","运算符 ""{0}"" 不能应用于类型 ""{1}""。",运算符“{0}”不能应用于类型“{1}”和“{2}”。,运算符不得混合在字符类中。改为将其包装在嵌套类中。,在编辑时选择项目退出多项目引用检查。,选项“{0}={1}”已删除。请从配置中删除它。,选项“{0}={1}”已弃用，并将停止在 TypeScript {2} 中运行。指定 compilerOption“ignoreDeprecations”:“{3}”以使此错误静音。,"选项“{0}”只能在 ""tsconfig.json"" 文件中指定，或者在命令行上设置为 ""false"" 或 ""null""。","选项“{0}”只能在 ""tsconfig.json"" 文件中指定或在命令行上设置为 ""null""。",选项“{0}”只能在命令行上指定。,"仅当提供了选项 ""--inlineSourceMap"" 或选项 ""--sourceMap"" 时，才能使用选项“{0}”。",只有在“moduleResolution”设置为“node16”、“nodenext”或“bundler”时，才能使用选项“{0}”。,只有在“module”设置为“preserve”或“es2015”或更高版本时，才能使用选项“{0}”。,"选项 ""jsx"" 为“{1}”时，不能指定选项“{0}”。",选项“{0}”不能与选项“{1}”同时指定。,无法在不指定选项“{1}”的情况下指定选项“{0}”。,无法在不指定选项 {1} 或选项 {2} 的情况下指定选项 {0}。,选项“{0}”已删除。请从配置中删除它。,选项“{0}”已弃用，并将停止在 TypeScript {1} 中运行。指定 compilerOption“ignoreDeprecations”:“{2}”以使此错误静音。,选项“{0}”是冗余选项，不能与选项“{1}”同时指定。,只有在设置“noEmit”或“emitDeclarationOnly”时，才能使用选项“allowImportingTsExtensions”。,选项 '--build' 必须是第一个命令行参数。,选项 “--incremental” 只能使用 tsconfig 指定，在发出到单个文件时指定，或在指定了选项 “--tsBuildInfoFile” 时指定。,"选项 ""isolatedModules"" 只可在提供了选项 ""--module"" 或者选项 ""target"" 是 ""ES2015"" 或更高版本时使用。",当选项“module”设置为“{1}”时，选项“moduleResolution”必须设置为“{0}”(或保留为未指定)。,当选项“moduleResolution”设置为“{1}”时，选项“module”必须设置为“{0}”。,启用“{0}”时，无法禁用选项“preserveConstEnums”。,"选项 ""project"" 在命令行上不能与源文件混合使用。",当“moduleResolution”设置为“classic”时，无法指定选项“--resolveJsonModule”。,当“module”设置为“none”、“system”或“umd”时，无法指定选项“--resolveJsonModule”。,当“module”设置为“UMD”、“AMD”或“System”时，不能使用选项“verbatimModuleSyntax”。,选项“{0}”与“{1}”不能组合在一起。,选项:,其他输出,输出格式设置,生成后输出编译器性能信息。,已生成声明文件的输出目录。,未从源文件“{1}”生成输出文件“{0}”。,"由于指定了 ""{1}""，因此包含了引用的项目 ""{0}"" 的输出","由于已将 ""--module"" 指定为 ""none""，因此包含了引用的项目 ""{0}"" 的输出",生成后输出更详细的编译器性能信息。,第 {0} 个重载(共 {1} 个)，“{2}”，出现以下错误。,重载签名必须都是抽象的或都是非抽象的。,重载签名必须全部为环境签名或非环境签名。,重载签名必须均导出或均不导出。,重载签名必须全部为可选签名或必需签名。,重载签名必须全部是公共签名、私有签名或受保护签名。,参数“{0}”不能引用在它之后声明的标识符“{1}”。,参数“{0}”不能引用它自身。,参数“{0}”隐式具有“{1}”类型。,"参数 ""{0}"" 隐式具有 ""{1}"" 类型，但可以从用法中推断出更好的类型。",参数“{0}”和参数“{1}”的位置不一样。,"访问器的参数 ""{0}"" 具有或正在使用外部模块 ""{2}"" 中的名称 ""{1}"" ，但不能为其命名。","访问器的参数 ""{0}"" 具有或正在使用专用模块 ""{2}"" 中的名称 ""{1}"" 。","访问器的参数 ""{0}"" 具有或正在使用专用名称 ""{1}""。",导出接口中的调用签名的参数“{0}”具有或正在使用私有模块“{2}”中的名称“{1}”。,导出接口中的调用签名的参数“{0}”具有或正在使用专用名称“{1}”。,导出类中的构造函数的参数“{0}”具有或正在使用外部模块“{2}”中的名称“{1}”，但不能为其命名。,导出类中的构造函数的参数“{0}”具有或正在使用私有模块“{2}”中的名称“{1}”。,导出类中的构造函数的参数“{0}”具有或正在使用专用名称“{1}”。,导出接口中的构造函数签名的参数“{0}”具有或正在使用私有模块“{2}”中的名称“{1}”。,导出接口中的构造函数签名的参数“{0}”具有或正在使用专用名称“{1}”。,导出函数的参数“{0}”具有或正在使用外部模块 {2} 中的名称“{1}”，但不能为其命名。,导出函数的参数“{0}”具有或正在使用私有模块“{2}”中的名称“{1}”。,导出函数的参数“{0}”具有或正在使用专用名称“{1}”。,来自导出接口的索引签名的参数“{0}”具有或正在使用来自私有模块“{2}”的名称“{1}”。,来自导出接口的索引签名的参数“{0}”具有或正在使用专用名称“{1}”。,导出接口中的方法的参数“{0}”具有或正在使用私有模块“{2}”中的名称“{1}”。,导出接口中的方法的参数“{0}”具有或正在使用专用名称“{1}”。,导出类中的公共方法的参数“{0}”具有或正在使用外部模块“{2}”中的名称“{1}”，但不能为其命名。,导出类中的公共方法的参数“{0}”具有或正在使用私有模块“{2}”中的名称“{1}”。,导出类中的公共方法的参数“{0}”具有或正在使用专用名称“{1}”。,导出类中的公共静态方法的参数“{0}”具有或正在使用外部模块“{2}”中的名称“{1}”，但不能为其命名。,导出类中的公共静态方法的参数“{0}”具有或正在使用私有模块“{2}”中的名称“{1}”。,导出类中的公共静态方法的参数“{0}”具有或正在使用专用名称“{1}”。,参数不能包含问号和初始化表达式。,应为参数声明。,"参数具有名称，但不具有类型。你是想使用 ""{0}: {1}"" 吗?",参数修饰符只能在 TypeScript 文件中使用。,参数必须具有带有 --isolatedDeclarations 的显式类型注释。,导出类中的公共 setter“{0}”的参数类型具有或正在使用私有模块“{2}”中的名称“{1}”。,导出类中的公共 setter“{0}”的参数类型具有或正在使用专用名称“{1}”。,导出类中的公共静态 setter“{0}”的参数类型具有或正在使用私有模块“{2}”中的名称“{1}”。,导出类中的公共静态 setter“{0}”的参数类型具有或正在使用专用名称“{1}”。,"以严格模式进行分析，并为每个源文件发出 ""use strict"" 指令。","tsconfig.js 中 ""files"" 列表的一部分","模式“{0}”最多只可具有一个 ""*"" 字符。","""--diagnostics"" 或 ""--extendedDiagnostics"" 的性能计时在此会话中不可用。未能找到 Web 性能 API 的本机实现。",平台特定,带下划线的前缀“{0}”,"使用 ""declare"" 作为所有错误的属性声明的前缀","尽可能在所有未使用的声明前添加前缀 ""_""","使用 ""declare"" 前缀",保留 JavaScript 输出中未使用的导入值，否则将删除这些值。,打印在编译过程中读取的所有文件。,打印在编译过程中读取的文件，包括包含它的原因。,打印文件的名称及编译包含这些文件的原因。,属于编译一部分的文件的打印名称。,打印编译包含的文件的名称，然后停止处理。,属于编译一部分的已生成文件的打印名称。,打印编译器的版本。,打印最终配置而不是生成。,编译后打印已发出文件的名称。,打印此消息。,定义了专用访问器，但没有 Getter。,必须在封闭类中声明私有字段“{0}”。,不允许在变量声明中使用专用标识符。,不允许在类主体之外使用专用标识符。,"专用标识符仅允许在类主体中使用，并且只能用作类成员声明的一部分、属性访问或用在 ""in"" 表达式的左侧",专用标识符仅在面向 ECMAScript 2015 和更高版本时可用。,不能将专用标识符用作参数。,"不能在类型参数上访问专用或受保护的成员 ""{0}""。",无法生成项目“{0}”，因为其依赖项“{1}”有错误,"无法生成项目 ""{0}"" ，因为未生成其依赖项 ""{1}""",正在强制重新生成项目“{0}”,由于 {1}，项目“{0}”已过期。,由于 buildinfo 文件“{1}”指示文件“{2}”曾是编译的根文件，但不再是了，因此项目“{0}”已过期。,由于 buildinfo 文件“{1}”指示程序需要报告错误，因此项目“{0}”已过期。,项目“{0}”已过期，因为 buildinfo 文件“{1}”指示某些更改未发出,由于 buildinfo 文件“{1}”指示 compilerOptions 中存在更改，因此项目“{0}”已过期,项目“{0}”已过期，因为其依赖项“{1}”已过期,项目“{0}”已过期，因为输出“{1}”早于输入“{2}”,项目“{0}”已过期，因为输出文件“{1}”不存在,"项目 ""{0}"" 已过期，因为其输出是使用与当前版本 ""{2}"" 不同的版本 ""{1}"" 生成的",项目“{0}”已过期，因为读取文件“{1}”时出错,“{0}”项目已是最新,项目“{0}”是最新的，因为最新的输入“{1}”早于输出“{2}”,项目“{0}”是最新的，但需要更新早于输入文件的输出文件的时间戳,项目“{0}”已是最新，拥有来自其依赖项的 .d.ts 文件,项目引用不能形成环形图。检测到循环: {0},项目,此生成中的项目: {0},"只有在面向 ECMAScript 2015 及更高版本时，才可使用带有 ""accessor"" 修饰符的属性。",属性“{0}”不能具有初始化表达式，因为它标记为摘要。,属性“{0}”来自索引签名，因此必须使用[“{0}”]访问它。,类型“{1}”上不存在属性“{0}”。,属性“{0}”在类型“{1}”上不存在。你是否指的是“{2}”?,属性“{0}”在类型“{1}”上不存在。你的意思是改为访问静态成员“{2}”吗?,属性“{0}”在类型“{1}”上不存在。是否需要更改目标库? 请尝试将 “lib” 编译器选项更改为“{2}”或更高版本。,属性“{0}”在类型 “{1}” 上不存在。请尝试将 “lib” 编译器选项更改为包含 “dom”。,属性“{0}”没有初始化表达式，并且未在类静态块中明确分配。,属性“{0}”没有初始化表达式，且未在构造函数中明确赋值。,"属性“{0}”隐式具有类型 ""any""，因为其 get 访问器缺少返回类型批注。","属性“{0}”隐式具有类型 ""any""，因为其 set 访问器缺少参数类型批注。","属性 ""{0}"" 隐式具有类型 ""any""，但可从用法为其 get 访问器推断出更好类型。","属性 ""{0}"" 隐式具有类型 ""any""，但可从用法为其 set 访问器推断出更好的类型。",类型“{1}”中的属性“{0}”不可分配给基类型“{2}”中的同一属性。,类型“{1}”中的属性“{0}”不可分配给类型“{2}”。,"类型 ""{1}"" 中的属性 ""{0}"" 引用了不能从类型 ""{2}"" 内访问的其他成员。",已声明属性“{0}”，但从未读取其值。,属性“{0}”与索引签名不兼容。,类型“{1}”中缺少属性“{0}”。,"类型 ""{1}"" 中缺少属性 ""{0}""，但类型 ""{2}"" 中需要该属性。","属性 ""{0}"" 在类 ""{1}"" 外部不可访问，因为它具有专用标识符。",属性“{0}”在类型“{1}”中为可选，但在类型“{2}”中为必选。,属性“{0}”为私有属性，只能在类“{1}”中访问。,属性“{0}”在类型“{1}”中是私有属性，但在类型“{2}”中不是。,属性“{0}”受保护，只能通过类“{1}”的实例进行访问。这是类“{2}”的实例。,属性“{0}”受保护，只能在类“{1}”及其子类中访问。,属性“{0}”受保护，但类型“{1}”并不是从“{2}”派生的类。,属性“{0}”在类型“{1}”中受保护，但在类型“{2}”中为公共属性。,在赋值前使用了属性“{0}”。,"属性 ""{0}"" 在其初始化前已被使用。",类型“{1}”上不存在属性“{0}”。你是否是指“{2}”?,JSX 展开特性的“{0}”属性不能分配给目标属性。,导出匿名类类型的属性“{0}”可能不是私有或受保护的属性。,导出接口的属性“{0}”具有或正在使用私有模块“{2}”中的名称“{1}”。,导出接口的属性“{0}”具有或正在使用专用名称“{1}”。,类型“{1}”的属性“{0}”不能赋给“{2}”索引类型“{3}”。,"属性 ""{0}"" 也在此处声明。","属性 ""{0}"" 将覆盖 ""{1}"" 中的基属性。如果是有意的，请添加初始值设定项。否则，请添加 ""declare"" 修饰符或删除多余的声明。",应为属性分配。,应为属性析构模式。,属性必须具有带有 --isolatedDeclarations 的显式类型注释。,应为属性或签名。,"属性值只能是字符串、数字、""true""、""false""、""null""、对象或数组等类型的字面量。",以“ES5”设为目标时，对“for-of”、传播和析构中的可迭代项提供完全支持。,导出类的公共方法“{0}”具有或正在使用外部模块“{2}”中的名称“{1}”，但不能为其命名。,导出类的公共方法“{0}”具有或正在使用私有模块“{2}”中的名称“{1}”。,导出类的公共方法“{0}”具有或正在使用专用名称“{1}”。,导出类的公共属性“{0}”具有或正在使用外部模块“{2}”中的名称“{1}”，但不能为其命名。,导出类的公共属性“{0}”具有或正在使用私有模块“{2}”中的名称“{1}”。,导出类的公共属性“{0}”具有或正在使用专用名称“{1}”。,导出类的公共静态方法“{0}”具有或正在使用外部模块“{2}”中的名称“{1}”，但不能为其命名。,导出类的公共静态方法“{0}”具有或正在使用私有模块“{2}”中的名称“{1}”。,导出类的公共静态方法“{0}”具有或正在使用专用名称“{1}”。,导出类的公共静态属性“{0}”具有或正在使用外部模块“{2}”中的名称“{1}”，但不能为其命名。,导出类的公共静态属性“{0}”具有或正在使用外部模块“{2}”中的名称“{1}”。,导出类的公共静态属性“{0}”具有或正在使用专用名称“{1}”。,"不允许使用限定名 ""{0}""，因为没有前导 ""@param {object} {1}""。",在未读取函数参数时引发错误。,"对具有隐式 ""any"" 类型的表达式和声明引发错误。","在带隐式“any"" 类型的 ""this"" 表达式上引发错误。",字符类中的范围顺序不对。,启用“{0}”时重新导出类型需要使用“导出类型”。,React 组件不能包含 JSX 命名空间名称,建议的选项,将输出结构重定向到目录。,减少 TypeScript 自动加载的项目数。,引用的项目“{0}”可能不会禁用发出。,"引用的项目“{0}”必须拥有设置 ""composite"": true。","通过 ""{0}"" 从文件 ""{1}"" 引用","当 ""--moduleResolution"" 为 ""node16"" 或 ""nodenext"" 时，相对导入路径需要 ECMAScript 导入中的显式文件扩展名。请考虑将扩展名添加到导入路径中。","当“--moduleResolution”为“node16” 或“nodenext”时，相对导入路径需要 ECMAScript 导入中的显式文件扩展名。你是想使用 ""{0}"" 吗?",从监视进程中删除目录列表。,从监视模式的处理中删除文件列表。,"删除所有不必要的 ""override"" 修饰符","删除 ""await"" 的所有不必要的使用",删除所有无法访问的代码,删除所有未使用的标签,从所有带有相关问题的箭头函数主体中删除大括号,从箭头函数中删除大括号,从箭头函数主体中删除大括号,从“{0}”删除导入,"删除 ""override"" 修饰符",删除括号,删除模板标记,删除 TypeScript 语言服务器中 JavaScript 文件总源代码大小 20mb 的上限。,从“{0}”中删除导入声明中的“type”,从“{1}”中删除“{0}”导入中的“type”,删除类型参数,"删除不必要的 ""await""",删除无法访问的代码,"为 ""{0}"" 删除未使用的声明",为“{0}”删除未使用的声明,删除未使用的解构声明,删除未使用的标签,删除变量语句,将 “@param” 标记名称“{0}”重命名为“{1}”,"将 ""{0}"" 替换为 ""Promise<{1}>""","将所有未使用的 ""infer"" 替换为 ""unknown""",用“{0}”替换导入。,"将 ""infer {0}"" 替换为 ""unknown""",在函数中的所有代码路径并非都返回值时报告错误。,报告 switch 语句中遇到 fallthrough 情况的错误。,.js 文件中的报表出错。,报告未使用的局部变量上的错误。,报告未使用的参数上的错误。,导出时需要足够的注释，以便其他工具可以轻松生成声明文件。,要求索引签名中有未声明的属性以使用元素访问。,所需的类型参数可能不遵循可选类型参数。,在位置“{1}”的缓存中找到模块“{0}”的解析。,在位置“{1}”的缓存中找到类型引用指令“{0}”的解析。,解析非相对名称失败；正在尝试禁用新式节点解析功能，以查看 npm 库是否需要配置更新。,解析非相对名称失败；正在尝试“--moduleResolution 捆绑程序”，以查看项目是否可能需要配置更新。,"只将 ""keyof"" 解析为字符串值的属性名称(不含数字或符号)。",已在条件“{0}”下解析。,正在 {0} 模式下解析，条件为 {1}。,======== 正在从“{1}”解析模块“{0}”。========,正在相对于基 URL“{1}”-“{2}”解析模块名“{0}”。,正在解析“{0}”的真实路径，结果为“{1}”。,======== 正在解析类型引用指令“{0}”，包含文件“{1}”。========,======== 正在解析类型引用指令“{0}”，包含文件“{1}”，根目录“{2}”。========,======== 正在解析类型引用指令“{0}”，包含文件“{1}”，未设置根目录。========,======== 正在解析类型引用指令“{0}”，未设置包含文件，根目录“{1}”。========,======== 正在解析类型引用指令“{0}”，未设置包含文件，未设置根目录。========,正在解析指定自定义 typeRoots 的程序的类型引用指令，跳过在“node_modules”文件夹中查找。,正在使用主搜索路径“{0}”解析。,"Rest 参数“{0}”隐式具有 ""any[]"" 类型。","Rest 参数 ""{0}"" 隐式具有 ""any[]"" 类型，但可从用法中推断出更好的类型。",rest 类型只能从对象类型创建。,返回类型注释循环引用自身。,必须从函数中推断返回类型,导出接口中的调用签名的返回类型具有或正在使用私有模块“{1}”中的名称“{0}”。,导出接口中的调用签名的返回类型具有或正在使用专用名称“{0}”。,导出接口中的构造函数签名的返回类型具有或正在使用私有模块“{1}”中的名称“{0}”。,导出接口中的构造函数签名的返回类型具有或正在使用专用名称“{0}”。,构造函数签名的返回类型必须可赋给类的实例类型。,导出函数的返回类型具有或正在使用外部模块“{1}”中的名称“{0}”，但不能为其命名。,导出函数的返回类型具有或正在使用私有模块“{1}”中的名称“{0}”。,导出函数的返回类型具有或正在使用专用名称“{0}”。,导出接口中的索引签名的返回类型具有或正在使用私有模块“{1}”中的名称“{0}”。,导出接口中的索引签名的返回类型具有或正在使用专用名称“{0}”。,导出接口中的方法的返回类型具有或正在使用私有模块“{1}”中的名称“{0}”。,导出接口中的方法的返回类型具有或正在使用专用名称“{0}”。,导出类中的公共 getter“{0}”的返回类型具有或正在使用外部模块“{2}”中的名称“{1}”，但不能为其命名。,导出类中的公共 getter“{0}”的返回类型具有或正在使用私有模块“{2}”中的名称“{1}”。,导出类中的公共 getter“{0}”的返回类型具有或正在使用专用名称“{1}”。,导出类中的公共方法的返回类型具有或正在使用外部模块“{1}”中的名称“{0}”，但不能为其命名。,导出类中的公共方法的返回类型具有或正在使用私有模块“{1}”中的名称“{0}”。,导出类中的公共方法的返回类型具有或正在使用专用名称“{0}”。,导出类中的公共静态 getter“{0}”的返回类型具有或正在使用外部模块“{2}”中的名称“{1}”，但不能为其命名。,导出类中的公共静态 getter“{0}”的返回类型具有或正在使用私有模块“{2}”中的名称“{1}”。,导出类中的公共静态 getter“{0}”的返回类型具有或正在使用专用名称“{1}”。,导出类中的公共静态方法的返回类型具有或正在使用外部模块“{1}”中的名称“{0}”，但不能为其命名。,导出类中的公共静态方法的返回类型具有或正在使用私有模块“{1}”中的名称“{0}”。,导出类中的公共静态方法的返回类型具有或正在使用专用名称“{0}”。,正在重用从位置“{2}”缓存中找到的“{1}”中模块“{0}”的解析，但其未解析。,正在重用从位置“{2}”缓存中找到的“{1}”中模块“{0}”的解析，已成功将其解析为“{3}”。,正在重用从位置“{2}”缓存中找到的“{1}”中模块“{0}”的解析，已成功将其解析为包 ID 为“{4}”的“{3}”。,正在重用旧程序“{1}”中模块“{0}”的解析，但其未解析。,正在重用旧程序“{1}”中模块“{0}”的解析，已成功将其解析为“{2}”。,正在重用旧程序“{1}”中模块“{0}”的解析，已成功将其解析为包 ID 为“{3}”的“{2}”。,正在重用从位置“{2}”缓存中找到的“{1}”中类型引用指令“{0}”的解析，但其未解析。,正在重用从位置“{2}”缓存中找到的“{1}”中类型引用指令“{0}”的解析，已成功将其解析为“{3}”。,正在重用从位置“{2}”缓存中找到的“{1}”中类型引用指令“{0}”的解析，已成功将其解析为包 ID 为“{4}”的“{3}”。,正在重用旧程序“{1}”中类型引用指令“{0}”的解析，但其未解析。,正在重用旧程序“{1}”中类型引用指令“{0}”的解析，已成功将其解析为“{2}”。,正在重用旧程序“{1}”中类型引用指令“{0}”的解析，已成功将其解析为包 ID 为“{3}”的“{2}”。,全部重写为索引访问类型,重写为索引访问类型“{0}”,将相对导入路径中的 ‘.ts’、‘.tsx’、‘.mts’ 和 ‘.cts’ 文件扩展名改写为其在输出文件中的 JavaScript 等效项。,由于左操作数永远不会为空，因此 ?? 的右操作数无法访问。,无法确定根目录，正在跳过主搜索路径。,为编译指定的根文件,策略,保存 .tsbuildinfo 文件以允许项目增量编译。,看到了不匹配的条件“{0}”。,检测到范围包，请在“{0}”中查看,正在搜索所有上级 node_modules 目录以查找回退扩展: {0}。,正在搜索所有上级 node_modules 目录以查找首选扩展: {0}。,另请参阅 https://aka.ms/tsconfig/module,所选内容不是有效的语句,所选内容不是有效的类型节点,为发出的 JavaScript 设置 JavaScript 语言版本并包含兼容的库声明。,设置来自 TypeScript 的消息传递的语言。这不会影响发出。,"将配置文件中的 ""module"" 选项设置为 ""{0}""",设置发出文件的换行符。,"将配置文件中的 ""target"" 选项设置为 ""{0}""",Setter 不能返回值。,显示所有编译器选项。,显示诊断信息。,显示详细的诊断信息。,显示将生成(如果指定有 '--clean'，则删除)什么,签名“{0}”必须为类型谓词。,签名声明只能在 TypeScript 文件中使用。,跳过在上游项目出错时生成下游项目。,跳过对所有 .d.ts 文件的类型检查。,跳过 TypeScript 附带的类型检查 .d.ts 文件。,跳过声明文件的类型检查。,正在跳过项目“{0}”的生成，因为其依赖项“{1}”有错误,"即将跳过项目 ""{0}"" 的生成，因为未生成其依赖项 ""{1}""",正在跳过看起来像绝对 URI、目标文件类型的模块“{0}”: {1}。,"由于指定了 ""{1}""，因此包含了引用的项目 ""{0}"" 的源","由于已将 ""--module"" 指定为 ""none""，因此包含了引用的项目 ""{0}"" 的源",源具有 {0} 个元素，但目标仅允许 {1} 个。,源具有 {0} 个元素，但目标需要 {1} 个。,源不提供目标中位置 {0} 处所需元素的匹配项。,源不提供目标中位置 {0} 处可变元素的匹配项。,指定 ECMAScript 目标版本。,指定 JSX 代码生成。,指定将所有输出捆绑到一个 JavaScript 文件中的文件。如果 “declaration” 为 true，还要指定一个捆绑所有 .d.ts 输出的文件。,指定与要包含在编译中的文件匹配的 glob 模式列表。,指定要包括的语言服务插件列表。,指定一组描述目标运行时环境的捆绑库声明文件。,指定一组将导入重新映射到其他查找位置的条目。,指定为项目指定路径的对象数组。在项目引用中使用。,为所有已发出的文件指定输出文件夹。,指定仅用于类型的导入的发出/检查行为。,指定用于存储增量编译信息的文件,指定 TypeScript 如何从给定的模块说明符查找文件。,指定在缺少递归文件监视功能的系统上监视目录的方式。,指定 TypeScript 监视模式的工作方式。,指定要在编译中包括的库文件。,指定模块代码生成。,指定使用 “jsx: react-jsx*” 时用于导入 JSX 中心函数的模块说明符。,指定多个行为类似于 “./node_modules/@types” 的文件夹。,指定对从中继承设置的基本配置文件的一个或多个路径或节点模块引用。,指定用于自动获取声明文件的选项。,"指定在使用文件系统事件创建轮询监视失败时创建轮询监视的策略: ""FixedInterval"" (默认)、""PriorityInterval""、""DynamicPriority""、""FixedChunkSize""。","指定在不支持本机递归监视的平台上监视目录的策略: ""UseFsEvents"" (默认)、""FixedPollingInterval""、""DynamicPriorityPolling""、""FixedChunkSizePolling""。","指定监视文件的策略: ""FixedPollingInterval"" (默认)、""PriorityPollingInterval""、""DynamicPriorityPolling""、""FixedChunkSizePolling""、""UseFsEvents""、""UseFsEventsOnParentDirectory""。",指定在将 React JSX 发出设定为目标时用于片段的 JSX 片段引用，例如 “React.Fragment” 或 “Fragment”。,"指定在设定 ""react"" JSX 发出目标时要使用的 JSX 工厂函数，例如 ""react.createElement"" 或 ""h""。",指定在将 React JSX 发出设定为目标时要使用的 JSX 中心函数，例如 “react.createElement” 或 “h”。,"当指定使用 ""jsxFactory"" 编译器选项面向 ""react"" JSX 发出时，指定要使用的 JSX 片段工厂函数，例如 ""Fragment""。",指定基目录以解析非相关模块名称。,"指定发出文件时要使用的行序列结尾: ""CRLF"" (dos)或 ""LF"" (unix)。",指定调试调试程序应将 TypeScript 文件放置到的位置而不是源位置。,指定调试程序应将映射文件放置到的位置而不是生成的位置。,指定用于从 “node_modules” 检查 JavaScript 文件的最大文件夹深度。仅适用于 “allowJs”。,"指定要用于从 eg,react 中导入 “jsx” 和 “jsxs” 工厂函数的模块说明符",指定为 “createElement” 调用的对象。这仅适用于将 “react” JSX 发出设定为目标的情况。,指定已生成声明文件的输出目录。,指定 .tsbuildinfo 增量编译文件的路径。,指定输入文件的根目录。与 --outDir 一起用于控制输出目录结构。,指定源文件中的根文件夹。,指定调试程序的根路径以查找引用源代码。,指定要包含的类型包名称，而无需在源文件中引用。,指定生成的 JSX 代码。,指定当系统耗尽本机文件观察程序时，观察程序应使用的方法。,指定生成的模块代码。,拆分所有无效的仅类型导入,拆分为两个单独的导入声明,"仅当面向 ECMAScript 5 和更高版本时，""new"" 表达式中的展开运算符才可用。",spread 类型只能从对象类型创建。,在监视模式下开始编译...,应为语句。,不允许在环境上下文中使用语句。,静态成员不能引用类类型参数。,静态属性“{0}”与构造函数“{1}”的内置属性函数“{0}”冲突。,更严格的类型检查选项,应为字符串字面量。,当“--module”标志设置为“es2015”或“es2020”时，不支持字符串文本导入和导出名称。,应为带双引号的字符串字面量。,样式选项,使用颜色和上下文风格化错误和消息(实验)。,当有减号时，子空间标志必须存在。,后续属性声明必须属于同一类型。属性“{0}”的类型必须为“{1}”，但此处却为类型“{2}”。,后续变量声明必须属于同一类型。变量“{0}”必须属于类型“{1}”，但此处却为类型“{2}”。,"模式“{1}”的替换“{0}”类型不正确，应为 ""string""，实际为“{2}”。","模式 ""{1}"" 中的替代项 ""{0}"" 最多只能有一个 ""*"" 字符",模式“{0}”的替代应为数组。,模式“{0}”的替换模式不应为空数组。,已成功创建 tsconfig.json 文件。,不允许在构造函数外部或在构造函数内的嵌套函数中进行 Super 调用。,取消对对象字面量的多余属性检查。,抑制缺少索引签名的索引对象的 noImplicitAny 错误。,在对缺少索引签名的对象编制索引时，抑制 “noImplicitAny” 错误。,将每个误用的“{0}”切换到“{1}”,在不支持本机递归监视的平台上同步调用回调并更新目录观察程序的状态。,语法: {0},标记“{0}”至少需要“{1}”个参数，但 JSX 工厂“{2}”最多可提供“{3}”个。,可选链中不允许使用带有标记的模板表达式。,目标仅允许 {0} 个元素，但源中的元素可能更多。,目标仅允许 {0} 个元素，但源中的元素可能不够。,目标签名提供的自变量太少。预期为 {0} 个或更多，但实际为 {1} 个。,"""{0}"" 修饰符只能在 TypeScript 文件中使用。","“{0}”运算符不能应用于类型 ""symbol""。",“{0}”运算符不允许用于布尔类型。请考虑改用“{1}”。,"异步迭代器的 ""{0}"" 属性必须是方法。","迭代器的 ""{0}"" 属性必须是方法。","""Object"" 类型可分配给极少数其他类型。是否想要改用“任意”类型?",不能同时设置 Unicode (u) 标志和 Unicode Sets (v) 标志。,"无法在 ES5 中的箭头函数中引用 ""arguments"" 对象。请考虑使用标准函数表达式。","无法在 ES5 中的异步函数或方法中引用 ""arguments"" 对象。请考虑使用标准函数或方法。","""if"" 语句的正文不能为空语句。",针对此实现的调用已成功，但重载的实现签名在外部不可见。,输入文件的字符集。,"包含箭头函数捕获 ""this"" 的全局值。",包含函数或模块体对于控制流分析而言太大。,当前文件是 CommonJS 模块，因此不能在顶级使用 “await”。,"当前文件是 CommonJS 模块，其导入将生成“require”调用；但是，引用的文件是 ECMAScript 模块，它不能使用“require”进行导入。请考虑改为编写动态“import(""{0}"")”调用。",当前主机不支持“{0}”选项。,"你可能打算使用的 ""{0}"" 的声明在此处定义",该声明曾在此处标记为已弃用。,"所需类型来自属性 ""{0}""，在此处的 ""{1}"" 类型上声明该属性",所需类型来自此签名的返回类型。,所需类型来自此索引签名。,导出分配的表达式必须是环境上下文中的标识符或限定的名称。,程序包含该文件是因为:,"配置文件“{0}”中的 ""files"" 列表为空。",在此处显示第一个导出默认值。,"承诺的 ""then"" 方法的第一个参数必须是回调。","全局类型 ""JSX.{0}"" 不可具有多个属性。",在此处声明实现签名。,将生成到 CommonJS 输出的文件中不允许 'import.meta' 元属性。,仅当 '--module' 选项为 'es2020'、'es2022'、'esnext'、'system'、'node16'、'node18'、'node20' 或 'nodenext' 时，才允许使用 'import.meta' 元属性。,"如果没有引用 ""{1}""，则无法命名 ""{0}"" 的推断类型。这很可能不可移植。需要类型注释。",推断类型“{0}”引用的类型具有无法简单序列化的循环结构。必须具有类型注释。,“{0}”的推断类型引用不可访问的“{1}”类型。需要类型批注。,此节点的推断类型超出编译器将序列化的最大长度。需要显式类型注释。,"""using"" 声明的初始值设定项必须是具有 ""[Symbol.dispose]()"" 方法的对象，或为 ""null"" 或 ""undefined""。","""await using"" 声明的初始值设定项必须是具有 ""[Symbol.asyncDispose]()"" 或 ""[Symbol.dispose]5D;()"" 方法的对象，或者是 ""null"" 或 ""undefined""。",由于属性“{1}”存在于多个要素中，但在某些要素中是专用属性，因此已将交集“{0}”缩减为“绝不”。,由于属性“{1}”在某些要素中具有存在冲突的类型，因此已将交集“{0}”缩减为“绝不”。,"""intrinsic"" 关键字只能用于声明编译器提供的内部类型。","必须提供 ""jsxFragmentFactory"" 编译器选项才能将 JSX 片段与 ""jsxFactory"" 编译器选项一起使用。",最后一个重载给出了以下错误。,在此处声明最后一个重载。,"""for...in"" 语句的左侧不能为析构模式。","""for...in"" 语句的左侧不能是 ""using"" 声明。","""for...in"" 语句的左侧不能是 ""await using"" 声明。","""for...in"" 语句的左侧不能使用类型批注。","""for…in"" 语句的左侧不能是可选属性访问。","""for...in"" 语句的左侧必须是变量或属性访问。","""for...in"" 语句的左侧必须是 ""string"" 或 ""any"" 类型。","""for...of"" 语句的左侧不能使用类型批注。","""for…of"" 语句的左侧不能是可选属性访问。",“for...of” 语句的左侧可能不是 “async”。,"""for...of"" 语句的左侧必须是变量或属性访问。","算术运算左侧必须是 ""any""、""number""、""bigint"" 或枚举类型。",赋值表达式的左侧不能是可选属性访问。,赋值表达式的左侧必须是变量或属性访问。,"""instanceof"" 表达式的左侧必须可分配给右侧 ""[Symbol.hasInstance]"" 方法的第一个自变量。","""instanceof"" 表达式左侧必须是 ""any"" 类型、对象类型或类型参数。","向用户显示消息时所用的区域设置(例如，""en-us"")",用于搜索 node_modules 和加载 JavaScript 文件的最大依赖项深度。,"""delete"" 运算符的操作数不能是专用标识符。","""delete"" 运算符的操作数不能是只读属性。","""delete"" 运算符的操作数必须是属性引用。","""delete"" 运算符的操作数必须是可选的。",增量或减量运算符的操作数不能是可选属性访问。,递增或递减运算符的操作数必须是变量或属性访问。,分析器预期在此处找到与“{0}”标记匹配的“{1}”。,项目根不明确，但需要解析文件“{1}”中的导出映射项“{0}”。提供 `rootDir` 编译器选项以消除歧义。,项目根不明确，但仍需要解析文件“{1}”中的导入映射项“{0}”。提供 `rootDir` 编译器选项以消除歧义。,"无法在此类中的类型 ""{1}"" 上访问属性 ""{0}""，因为具有相同拼写的另一个专用标识符隐藏了它。","参数修饰器函数的返回类型必须为 ""void"" 或 ""any""。","属性修饰器函数的返回类型必须为 ""void"" 或 ""any""。","异步函数的返回类型必须是有效承诺，或不得包含可调用的 ""then"" 成员。 ",异步函数或方法的返回类型必须为全局 Promise<T> 类型。,"异步函数或方法的返回类型必须为全局 Promise<T> 类型。你是否是指写入 ""Promise<{0}>""?","""for...in"" 语句右侧必须是 ""any"" 类型、对象类型或类型参数，但此处的类型为“{0}”。","算术运算右侧必须是 ""any""、""number""、""bigint"" 或枚举类型。","""instanceof"" 表达式的右侧必须是类型 ""any""、类、函数或其他可分配给 ""Function"" 接口类型的类型，或者是具有 ""Symbol.hasInstance"" 方法的对象类型。","""instanceof"" 表达式的右侧不能是实例化表达式。",“{0}”文件的根值必须是一个对象。,运行时将使用 {1} 个自变量调用修饰器，但修饰器需要 {0} 个。,运行时将使用 {1} 个自变量调用修饰器，但修饰器至少需要 {0} 个。,在此处定义了“{0}”的阴影声明,“{1}”的签名“{0}”已弃用。,指定的路径不存在:“{0}”。,第一次在此处指定了标记。,对象 rest 分配的目标不能是可选属性访问。,对象 rest 分配的目标必须是变量或属性访问。,"类型为“{0}”的 ""this"" 上下文不能分配给类型为“{1}”的方法的 ""this""。","每个签名的 ""this"" 类型不兼容。","类型 ""{0}"" 为 ""readonly""，不能分配给可变类型 ""{1}""。",在将 “export type” 用在其导出语句上时，不能在已命名导出上使用 “type” 修饰符。,在将 “import type” 用在其导入语句上时，不能在已命名导入上使用 “type” 修饰符。。,函数声明的类型必须与函数的签名匹配。,无法序列化此节点的类型，因为无法序列化其属性“{0}”。,"异步迭代器的 ""{0}()"" 方法返回的类型必须是具有 ""value"" 属性的类型的承诺。","迭代器的 ""{0}()"" 方法返回的类型必须具有 ""value"" 属性。","在这些类型中，""{0}"" 的类型不兼容。","在这些类型中，""{0}"" 返回的类型不兼容。",此处不能使用值“{0}”。,"""for...in"" 语句的变量声明不能有初始化表达式。","""for...of"" 语句的变量声明不能有初始化表达式。","不支持 ""with"" 语句。""with"" 程序块中的所有符号都将具有类型 ""any""。","“{0}”处有类型，但无法在当前 ""moduleResolution"" 设置下解析此结果。请考虑更新到 ""node16""、""nodenext"" 或 ""bundler""。","“{0}”处有类型，但在遵守 package.json ""exports"" 时无法解析此结果。“{1}”库可能需要更新其 package.json 或键入。",此正则表达式中没有名为“{0}”的捕获组。,没有可重复的内容。,此 JSX 标记要求 ‘{0}’ 在范围内，但找不到它。,此 JSX 标记要求模块路径 ‘{0}’ 存在，但找不到任何路径。请确保已安装相应包的类型。,"此 JSX 标记的 ""{0}"" 属性需要 ""{1}"" 类型的子级，但提供了多个子级。","此 JSX 标记的 ""{0}"" 属性需要类型 ""{1}""，该类型需要多个子级，但仅提供了一个子级。",此向后引用指的是一个不存在的组。此正则表达式中没有捕获组。,此向后引用指的是一个不存在的组。此正则表达式中只有 {0} 个捕获组。,此二进制表达式永远不会为 null。是否缺少括号?,不能在正则表达式中转义此字符。,此比较似乎是无意的，因为类型“{0}”和“{1}”没有重叠。,此条件将始终返回“{0}”。,此条件将始终返回“{0}”，因为 JavaScript 按引用而不是值比较对象。,此条件将始终返回 true，因为此“{0}”已始终定义。,此条件将始终返回 true，因为始终定义了函数。你是想改为调用它吗?,此构造函数可能会转换为类声明。,此表达式始终为 null。,此表达式从不为 null。,此表达式不可调用。,"此表达式是 ""get"" 访问器，因此不可调用。你想在不使用 ""()"" 的情况下使用它吗?",此表达式不可构造。,此文件已具有默认导出,重写此导入路径并不安全，因为它会解析为另一个项目，并且项目的输出文件之间的相对路径与其输入文件之间的相对路径不同。,此导入使用 ‘{0}’ 扩展解析为输入 TypeScript 文件，但不会在发出期间重写，因为它不是相对路径。,这是正在扩充的声明。请考虑将扩充声明移到同一个文件中。,这种表达式的结果始终为 false。,这种表达式的结果始终为 true。,可将此转换为异步函数。,此成员不能具有带 “@override” 标记的 JSDoc 注释，因为未在基类“{0}”中对其进行声明。,此成员不能具有带 “override” 标记的 JSDoc 注释，因为未在基类“{0}”中对其进行声明。你是否指的是“{1}”?,此成员不能具有带 “@override” 标记的 JSDoc 注释，因为所包含的类“{0}”不会扩展其他类。,此成员不能拥有带有 ‘@overrid’ 标记的 JSDoc 注释，因为其名称是动态的。,"此成员不能有 ""override"" 修饰符，因为它未在基类 ""{0}"" 中声明。",此成员不能有 “override” 修饰符，因为它未在基类“{0}”中声明。你是否指的是“{1}”?,"此成员不能有 ""override"" 修饰符，因为它的包含类 ""{0}"" 不扩展其他类。",此成员不能具有 “override” 修饰符，因为其名称是动态的。,此成员必须具有带 “@override” 标记的 JSDoc 注释，因为它会替代基类“{0}”中的成员。,"此成员必须有 ""override"" 修饰符，因为它替代基类 ""{0}"" 中的一个成员。","此成员必须有 ""override"" 修饰符，因为它替代基类 ""{0}"" 中声明的一个抽象方法。","只能通过启用 ""{0}"" 标志并引用其默认导出，使用 ECMAScript 导入/导出来引用此模块。",此模块是使用 “export =” 声明的，只能在使用“{0}”标志时用于默认导入。,可以简化此操作。此班次与 `{0} {1} {2}` 相同。,此重载隐式返回类型“{0}”，因为它缺少返回类型批注。,此重载签名与其实现签名不兼容。,"此参数不允许与 ""use strict"" 指令一起使用。",此参数属性必须具有带 “@override” 标记的 JSDoc 注释，因为它将替代基类“{0}”中的成员。,此参数属性必须具有 “override” 修饰符，因为它会替代基类“{0}”中的成员。,此正则表达式标志不能在子模式内切换。,此正则表达式标志仅在面向“{0}”或更高版本时可用。,重写此相对导入路径并不安全，因为它看起来像文件名，但实际上解析为 ‘{0}’。,此扩张将始终覆盖此属性。,启用 “erasableSyntaxOnly” 时，不允许使用此语法。,此语法保留在扩展名为 .mts 或 .cts 的文件中。请添加尾随逗号或显式约束。,此语法保留在扩展名为 .mts 或 .cts 的文件中。请改用 `as` 表达式。,此语法需要一个导入的帮助程序，但找不到模块“{0}”。,"此语法需要名为 ""{1}"" 的导入帮助器，""{0}"" 中不存在该帮助器。请考虑升级 ""{0}"" 的版本。","此语法需要一个名为 ""{1}"" 且包含 {2} 参数的导入帮助程序，该帮助程序与 ""{0}"" 中的相应帮助程序不兼容。请考虑升级 ""{0}"" 的版本。",此类型参数可能需要 `extends {0}` 约束。,“import” 的这种用法无效。可以写入 “import()” 调用，但它们必须具有括号，并且不能带有类型参数。,"若要将此文件转换为 ECMAScript 模块，请将字段“""type"": ""module""”添加到“{0}”。","若要将此文件转换为 ECMAScript 模块，请将其文件扩展名更改为“{0}”，或将字段“""type"": ""module""”添加到“{1}”。","若要将此文件转换为 ECMAScript 模块，请将其文件扩展名更改为“{0}”，或者使用“{ ""type"": ""module"" }”创建本地 package.json 文件。","若要将此文件转换为 ECMAScript 模块，请使用“{ ""type"": ""module"" }”创建本地 package.json 文件。",仅当 ‘module’ 选项设置为 ‘es2022’、‘esnext’、‘system’、‘node16’、‘node18’、‘node20’、‘nodenext’ 或 ‘preserve’，且 ‘target’ 选项设置为 ‘es2017’ 或更高版本时，才允许使用顶级 ‘await’ 表达式。,仅当 ‘module’ 选项设置为 ‘es2022’、‘esnext’、‘system’、‘node16’、‘node18’、‘node20’、‘nodenext’ 或 ‘preserve’ 且 ‘target’ 选项设置为 ‘es2017’ 或更高时，才允许使用顶级 ‘await using’ 语句。,".d.ts 文件中的顶级声明必须以 ""declare"" 或 ""export"" 修饰符开头。",仅当 ‘module’ 选项设置为 ‘es2022’、‘esnext’、‘system’、‘node16’、‘node18’、‘node20’、‘nodenext’ 或 ‘preserve’，且 ‘target’ 选项设置为 ‘es2017’ 或更高版本时，才允许使用顶级 ‘for await’ 循环。,不允许使用尾随逗号。,"将每个文件转换为单独的模块(类似 ""ts.transpileModule"")。",尝试使用 `npm i --save-dev @types/{1}` (如果存在)，或者添加一个包含 `declare module '{0}';` 的新声明(.d.ts)文件,"正在尝试 ""rootDirs"" 中的其他条目。",正在尝试替换“{0}”，候选模块位置:“{1}”。,"长度为 ""{1}"" 的元组类型 ""{0}"" 在索引 ""{2}"" 处没有元素。",元组类型参数循环引用自身。,"只有在使用 ""--downlevelIteration"" 标志或 ""--target"" 为 ""es2015"" 或更高版本时，才能循环访问类型“{0}”。",类型“{0}”不能作为索引类型使用。,类型“{0}”无法用于索引类型“{1}”。,类型“{0}”不满足约束“{1}”。,类型“{0}”不满足预期类型“{1}”。,"类型 ""{0}"" 没有调用签名。","类型 ""{0}"" 没有构造签名。",类型“{0}”没有匹配的类型“{1}”的索引签名。,类型“{0}”与类型“{1}”不具有相同的属性。,类型“{0}”没有类型参数列表适用的签名。,类型“{0}”是泛型的，只能编制索引以供读取。,类型“{0}”缺少类型“{1}”中的以下属性: {2},类型“{0}”缺少类型“{1}”的以下属性: {2} 及其他 {3} 项。,类型“{0}”不是构造函数类型。,类型“{0}”不是 ES5 中有效的异步函数返回类型，因为它不引用与 Promise 兼容的构造函数值。,类型“{0}”不是数组类型。,类型“{0}”不是数组类型或字符串类型。,"类型“{0}”不是数组类型或字符串类型，或者没有返回迭代器的 ""[Symbol.iterator]()"" 方法。","类型“{0}”不是数组类型，或者没有返回迭代器的 ""[Symbol.iterator]()"" 方法。",不能将类型“{0}”分配给类型“{1}”。,类型“{0}”不可分配给类型“{1}”。你的意思是“{2}”?,类型“{0}”无法分配给类型“{1}”。存在具有此名称的两种不同类型，但它们是不相关的。,类型“{0}”不能分配给类型“{1}”，如方差批注所示。,根据计算枚举成员值的要求，类型“{0}”不能分配给类型“{1}”。,类型 “{0}” 不能分配给“exactOptionalPropertyTypes: true”的类型 “{1}”。请考虑将 “undefined” 添加到目标属性的类型。,类型 “{0}” 不能分配给“exactOptionalPropertyTypes: true”的类型 “{1}”。请考虑将 “undefined” 添加到目标类型。。,类型“{0}”不可与类型“{1}”进行比较。,类型“{0}”不是泛型类型。,"类型 ""{0}"" 可以表示基元值，该值不允许作为“in”运算符的右操作数。","类型“{0}”必须具有返回异步迭代器的 ""[Symbol.asyncIterator]()"" 方法。","类型“{0}”必须具有返回迭代器的 ""[Symbol.iterator]()"" 方法。",类型“{0}”提供的内容与签名“{1}”不匹配。,类型“{0}”以递归方式将自身引用为基类。,类型检查,类型别名“{0}”循环引用自身。,必须为类型别名指定名称。,类型别名不能为“{0}”。,类型别名只能在 TypeScript 文件中使用。,类型批注不能出现在构造函数声明中。,类型注释只能在 TypeScript 文件中使用。,应为类型参数。,类型参数列表不能为空。,类型参数只能在 TypeScript 文件中使用。,"""{0}"" 的类型参数循环引用自身。",类型断言表达式只能在 TypeScript 文件中使用。,源中位置 {0} 的类型与目标中位置 {1} 的类型不兼容。,源中位置 {0} 到 {1} 的类型与目标中位置 {2} 的类型不兼容。,包含专用名称“{0}”的类型不能与 --isolatedDeclarations 一起使用。,要包含在编译中类型声明文件。,应为类型。,"类型导入断言应恰好有一个键 - ""resolution-mode"" - 值为 ""import"" 或 ""require""。","类型导入属性应只有一个键 ""resolution-mode""，值为 ""import"" 或 ""require""。","从 CommonJS 模块导入 ECMAScript 模块的类型导入必须具有 ""resolution-mode"" 属性。",类型实例化过深，且可能无限。,"类型在其自身的 ""then"" 方法的 fulfillment 回调中被直接或间接引用。","通过 ""{0}"" 从文件 ""{1}"" 引用了库类型","通过 ""{0}"" 从具有 packageId ""{2}"" 的文件 ""{1}"" 引用了库类型","""await"" 操作数的类型必须是有效承诺，或不得包含可调用的 ""then"" 成员。","计算属性类型的值为 ""{0}""，该值不能赋给 ""{1}"" 类型。",实例成员变量“{0}”的类型不能引用构造函数中声明的标识符“{1}”。,"""yield*"" 操作数的迭代元素的类型必须是有效承诺，或不得包含可调用的 ""then"" 成员。",属性“{0}”的类型在已映射的类型“{1}”中循环引用其自身。,"异步生成器中 ""yield"" 操作数的类型必须是有效承诺，或不得包含可调用的 ""then"" 成员。","从 CommonJS 模块导入 ECMAScript 模块的仅类型导入必须具有 ""resolution-mode"" 属性。",此导入产生的类型。无法调用或构造命名空间样式的导入，这类导入将在运行时导致失败。请考虑改为使用默认导入或此处需要的导入。,类型参数“{0}”具有循环约束。,类型参数“{0}”具有循环默认值。,导出接口中的调用签名的类型参数“{0}”具有或正在使用专用名称“{1}”。,导出接口中的构造函数签名的类型参数“{0}”具有或正在使用专用名称“{1}”。,导出类的类型参数“{0}”具有或正在使用专用名称“{1}”。,导出函数的类型参数“{0}”具有或正在使用专用名称“{1}”。,导出接口的类型参数“{0}”具有或正在使用专用名称“{1}”。,"已导出映射对象类型的类型参数 ""{0}"" 使用专用名称 ""{1}"" 。",已导出类型别名的类型参数“{0}”具有或正使用专用名称“{1}”。,导出接口中的方法的类型参数“{0}”具有或正在使用专用名称“{1}”。,导出类中的公共方法的类型参数“{0}”具有或正在使用专用名称“{1}”。,导出类中的公共静态方法的类型参数“{0}”具有或正在使用专用名称“{1}”。,应为类型参数声明。,类型参数声明只能在 TypeScript 文件中使用。,类型参数默认值只能引用以前声明的类型参数。,类型参数列表不能为空。,类型参数名称不能为“{0}”。,类型参数不能出现在构造函数声明中。,类型谓词“{0}”不可分配给“{1}”。,类型生成的元组类型太大，无法表示。,======== 未解析类型引用指令“{0}”。========,======== 类型引用指令“{0}”已成功解析为“{1}”，主要: {2}。========,"======== 类型引用指令 ""{0}"" 已成功解析为 ""{1}"" ，包 ID 为 ""{2}""，主要: {3}。========",类型满意度表达式只能在 TypeScript 文件中使用。,类型不能出现在 JavaScript 文件的导出声明中。,类型具有私有属性“{0}”的单独声明。,构造签名的类型不兼容。,参数“{0}”和“{1}” 的类型不兼容。,属性“{0}”的类型不兼容。,无法打开文件“{0}”。,作为表达式调用时，无法解析类修饰器的签名。,作为表达式调用时，无法解析方法修饰器的签名。,作为表达式调用时，无法解析参数修饰器的签名。,作为表达式调用时，无法解析属性修饰器的签名。,不确定的字符转义。,意外的“{0}”。你是否想要使用反斜杠对其进行转义?,文本意外结束。,意外的关键字或标识符。,意外的标记。,意外的标记。应为构造函数、方法、访问器或属性。,意外的标记。类型参数名不应包含大括号。,意外的标记。你是想使用 `{'>'}` 还是 `&gt;`?,意外的标记。你是想使用 `{'}'}` 还是 `&rbrace;`?,"意外标记。应为 ""{""。",Unicode 转义序列不能显示在此处。,仅当设置了 Unicode (u) 标志或 Unicode Sets (v) 标志时，Unicode 转义序列才可用。,仅当设置了 Unicode (u) 标志或 Unicode Sets (v) 标志时，Unicode 属性值表达式才可用。,未知的 Unicode 属性名称。,未知的 Unicode 属性名称或值。,未知的 Unicode 属性值。,"未知的生成选项 ""{0}""。","未知的生成选项 ""{0}""。你是想使用 ""{1}"" 吗?",未知的编译器选项“{0}”。,"未知的编译器选项 ""{0}""。你是想使用 ""{1}"" 吗?",未知的关键字或标识符。你是不是指“{0}”?,"未知的 ""excludes"" 选项。你的意思是 ""exclude""?",未知的正则表达式标志。,未知类型获取选项“{0}”。,"未知的类型获取选项 ""{0}""。你是想使用 ""{1}"" 吗?","未知观察选项 ""{0}""。","未知的监视选项 ""{0}""。你是想使用 ""{1}"" 吗?",检测到无法访问的代码。,未终止的 Unicode 转义序列。,响应文件“{0}”中引号不配对。,未终止的正则表达式字面量。,未终止的字符串字面量。,未终止的模板字面量。,非类型化函数调用不能接受类型参数。,未使用的标签。,"未使用的 ""@ts-expect-error"" 指令。",从“{0}”更新导入,更新“{0}”的修饰符,正在更新项目“{0}”的输出时间戳...,"正在更新项目 ""{0}"" 未更改的输出时间戳…",使用 `{0}`,请改用“{0}”。,在所有条件下使用 `Number.isNaN`。,对“{0}”使用元素访问,对所有未声明的属性使用元素访问。,"使用 ""import type""",使用综合的“默认”成员。,"解析包导入时，请使用 package.json ""exports"" 字段。","解析导入时，请使用 package.json ""import"" 字段。","使用 ""type {0}""",将“{0}”子路径“{1}”与目标“{2}”一起使用,使用 JSX 片段需要片段工厂 ‘{0}’ 在范围内，但找不到它。,"仅 ECMAScript 5 和更高版本支持在 ""for...of"" 语句中使用字符串。",使用 --build，-b 将使 tsc 的行为更像生成业务流程协调程序，而非编译器。这可用于触发生成复合项目，你可以在 {0} 详细了解这些项目,使用项目引用重定向“{0}”的编译器选项。,版本,类型“{0}”的值没有与类型“{1}”相同的属性。你是想调用它吗?,"类型“{0}”的值不可调用。是否希望包括 ""new""?",变量“{0}”隐式具有“{1}”类型。,"变量 ""{0}"" 隐式具有 ""{1}"" 类型，但可以从用法中推断出更好的类型。","变量 ""{0}"" 在某些位置隐式具有类型 ""{1}""，但可以从使用情况推断出更好的类型。",变量“{0}”在某些无法确定其类型的位置处隐式具有类型“{1}”。,在赋值前使用了变量“{0}”。,应为变量声明。,变量声明列表不能为空。,此位置不允许使用变量声明。,变量必须具有带有 --isolatedDeclarations 的显式类型注释。,具有多个声明的变量不能内联。,源中位置 {0} 的可变元素与目标中位置 {1} 的元素不匹配。,仅对象、函数、构造函数、映射类型的类型别名支持方差注释。,版本 {0},请访问 https://aka.ms/tsconfig，了解有关此文件的详细信息,监视选项,观看和生成模式,监视输入文件。,"观察选项 ""{0}"" 需要 {1} 类型的值。",我们只能通过在此处为整个参数添加类型来写入“{0}”的类型。,分配函数时，请检查以确保参数和返回值与子类型兼容。,进行类型检查时，请考虑 “null” 和 “undefined”。,是否在监视模式下保留过时的控制台输出，而不是清除屏幕。,包装表达式容器中的所有无效字符,将所有无效的修饰器表达式括在括号中,用括号括起所有对象字面量,将所有没有父级的 JSX 包装在 JSX 片段中,包装在 JSX 片段中,用括号括起来,包装表达式容器中的无效字符,用括号括起以下应为对象字面量的内容,你可以在 {0} 了解编译器选项的所有内容,不能通过全局导入对模块进行重命名。,不能重命名已在 “node_modules” 文件夹中定义的元素。,不能重命名已在另一个 “node_modules” 文件夹中定义的元素。,不能重命名标准 TypeScript 库中定义的元素。,无法重命名此元素。,"“{0}”收到的参数过少，无法在此处充当修饰器。你是要先调用它，然后再写入 ""@{0}()"" 吗?",“{0}”和“{1}”索引签名不兼容。,"不能在不使用括号的情况下混用 ""{0}"" 和 ""{1}"" 操作。",“{0}”被指定了两次。将覆盖名为“{0}”的特性。,类型末尾的“{0}”不是有效的 TypeScript 语法。是否要写入“{1}”?,类型开头的“{0}”不是有效的 TypeScript 语法。是否要写入“{1}”?,"只能通过启用 ""esModuleInterop"" 标志并使用默认导入来导入“{0}”。",仅可使用默认导入来导入“{0}”。,"只能通过使用 ""require"" 调用或启用 ""esModuleInterop"" 标志并使用默认导入来导入“{0}”。","只能使用 ""require"" 调用或使用默认导入来导入“{0}”。","仅可使用 ""import {1} = require({2})"" 或默认导入来导入“{0}”。","仅可使用 ""import {1} = require({2})"" 或通过启用 ""esModuleInterop"" 标志并使用默认导入来导入“{0}”。",“{0}”不能用作 JSX 组件。,"""{0}"" 是使用 ""export type"" 导出的，因此不能用作值。","""{0}"" 是使用 ""import type"" 导入的，因此不能用作值。","""{0}"" 组件不接受文本作为子元素。JSX 中的文本类型为 ""string""，但 ""{1}"" 的预期类型为 ""{2}""。",“{0}”可以使用与“{1}”无关的任意类型进行实例化。,“{0}”声明只能在块的内部声明。,"""{0}"" 声明只能在 TypeScript 文件中使用。",“{0}”声明可能没有绑定模式。,必须初始化“{0}”声明。,应为“{0}”。,"“{0}”具有字符串类型，但在启用 ""isolatedModules"" 时必须具有语法上可识别的字符串语法。",“{0}”没有导出的成员“{1}”。你是否指的是“{2}”?,"""{0}"" 隐式具有 ""{1}"" 返回类型，但可以从用法中推断出更好的类型。","由于“{0}'”不具有返回类型批注并且在它的一个返回表达式中得到直接或间接引用，因此它隐式具有返回类型 ""any""。","“{0}”隐式具有类型 ""any""，因为它不具有类型批注，且在其自身的初始化表达式中得到直接或间接引用。",“{0}”索引签名不兼容。,“{0}”索引类型“{1}”不能分配给“{2}”索引类型“{3}”。,“{0}”是基元，但“{1}”是包装器对象。如可能首选使用“{0}”。,“{0}”是一种类型，无法在 JavaScript 文件中导入。请在 JSDoc 类型批注中使用“{1}”。,"“{0}”是一种类型，必须在启用 ""verbatimModuleSyntax"" 时使用仅类型导入进行导入。",“{0}”是“{1}”的未使用重命名。是否打算将其用作类型批注?,"""{0}"" 可赋给 ""{1}"" 类型的约束，但可以使用约束 ""{2}"" 的其他子类型实例化 ""{1}""。",“{0}”自动导出到此处。,已声明“{0}”，但从未读取其值。,“{0}”已声明，但从未使用过。,"在此处声明了 ""{0}""。","""{0}"" 在类 ""{1}"" 中定义为属性，但这里在 ""{2}"" 中重写为访问器。","""{0}"" 在类 ""{1}"" 中定义为访问器，但这里在 ""{2}"" 中重写为实例属性。",“{0}”已弃用。,“{0}”不是关键字“{1}”的有效元属性。是否是指“{2}”?,“{0}”不是关键字 ‘import’ 的有效元属性。你是说 ‘meta’ 还是 ‘defer’?,不允许将 '{0}' 作为参数名。,“{0}”不得用作变量声明名称。,“{0}”的类型为“未知”。,“{0}”可能为 “null”。,{0}可能为 “null” 或“未定义”。,“{0}”可能为“未定义”。,“{0}”在其自身的基表达式中得到直接或间接引用。,“{0}”在其自身的类型批注中得到直接或间接引用。,"多次指定了 ""{0}""，因此将重写此用法。",“{0}”列表不能为空。,已看到“{0}”修饰符。,“{0}”修饰符只能出现在类、接口或类型别名的类型参数上,“{0}”修饰符只能出现在函数、方法或类的类型参数上,“{0}”修饰符不能出现在构造函数声明中。,“{0}”修饰符不可出现在模块或命名空间元素上。,“{0}”修饰符不能出现在参数中。,“{0}”修饰符不可出现在类型成员上。,“{0}”修饰符不能出现在类型参数上,"“{0}”修饰符不能出现在 ""using"" 声明中。","“{0}”修饰符不能出现在 ""await using"" 声明中。",“{0}”修饰符不可出现在索引签名上。,“{0}”修饰符不能出现在此类型的类元素上。,“{0}”修饰符不能在此处使用。,“{0}”修饰符不能在环境上下文中使用。,“{0}”修饰符不能与“{1}”修饰符一起使用。,“{0}”修饰符不能与专用标识符一起使用。,“{0}”修饰符必须位于“{1}”修饰符之前。,“\{0}”后面必须是括在大括号中的 Unicode 属性值表达式。,"""{0}"" 需要显式类型注释。",“{0}”仅指类型，但在此用作命名空间。,“{0}”仅表示类型，但在此处却作为值使用。,“{0}”仅引用一个类型，但在此处用作一个值。你是否想要使用“{0} 中的 {1}”?,“{0}” 仅指类型，但在此处用作值。是否需要更改目标库? 请尝试将 “lib” 编译器选项更改为 es2015 或更高版本。,“{0}”指 UMD 全局，但当前文件是模块。请考虑改为添加导入。,“{0}”表示值，但在此处用作类型。是否指“类型 {0}”?,"“{0}”解析为一个类型，并且在启用“{1}”后重新导出之前，必须在此文件中标记为仅类型。请考虑使用导入“{0}”的 ""import type""。","“{0}”解析为一个类型，并且在启用“{1}”后重新导出之前，必须在此文件中标记为仅类型。请考虑使用 ""export type { {0} as default }""。","“{0}”解析为仅类型声明，并且必须在启用 ""verbatimModuleSyntax"" 时使用仅类型导入进行导入。","“{0}”解析为仅类型声明，并且在启用“{1}”后重新导出之前，必须在此文件中标记为仅类型。请考虑使用导入“{0}”的 ""import type""。","“{0}”解析为仅类型声明，并且在启用“{1}”后重新导出之前，必须在此文件中标记为仅类型。请考虑使用 ""export type { {0} as default }""。",“{0}”解析为仅类型声明，并且在启用“{1}”时必须使用仅类型重新导出进行重新导出。,应在 config json 文件的 “compilerOptions” 对象中设置 “{0}”,已指定“{0}”标记。,"此处也声明了 ""{0}""。","在此处导出了 ""{0}""。","此处导入了 ""{0}""。",缺少返回类型批注的“{0}”隐式具有“{1}”返回类型。,"缺少返回类型批注的 ""{0}"" 隐式具有 ""{1}"" 产出类型。","""abstract"" 修饰符仅可出现在类、方法或属性声明中。","""accessor"" 修饰符只能出现在属性声明中。",并在这里。,和 npm 安装 -D @types/节点,无法在属性初始值设定项或类静态初始化块中引用 'arguments'。,“auto”: 将带有导入、导出、import.meta、jsx (带有 jsx: react-jsx)或 esm 格式(带模块: node16+)的文件视为模块。,"""await"" 表达式不能在类静态块内使用。","仅当文件是模块时，才允许在该文件的顶层使用 ""await"" 表达式，但此文件没有导入或导出。请考虑添加空的 ""export {}"" 以将此文件变为模块。","仅允许在异步函数和模块顶级使用 ""await"" 表达式。","不能在参数初始化表达式中使用 ""await"" 表达式。","""await"" 对此表达式的类型没有影响。",环境上下文中不允许使用 ‘await using’ 声明。,"只有当文件是模块时，才允许在该文件的顶层使用 ""await using"" 语句，但此文件没有导入或导出。可考虑添加空的 ""export {}"" 将此文件变为模块。","""await using"" 语句仅允许在异步函数和顶级模块中使用。","""await using"" 语句不能在类静态块内使用。","""baseUrl"" 选项设置为“{0}”，可使用此值解析非相关模块名称“{1}”。","""\c"" 后面必须有一个 ASCII 字母。","""#!"" 只能用在文件的开头。","应为 ""case"" 或 ""default""。",应为 “catch” 或 “finally”。,"""const"" 枚举成员初始化表达式的求值结果为非有限值。","""const"" 枚举成员初始化表达式的求值结果为不允许使用的值 ""NaN""。",const 枚举成员初始值设定项必须是常量表达式。,"""const"" 枚举仅可在属性、索引访问表达式、导入声明的右侧、导出分配或类型查询中使用。",“构造函数”不能用作参数属性名称。,"""#constructor"" 是保留字。",默认值:,"在严格模式下，无法对标识符调用 ""delete""。","""export *"" 不会重新导出默认值。","""export ="" 只能在 TypeScript 文件中使用。","""export"" 修饰符不可用于环境模块和模块扩大，因为它们始终可见。","已看到 ""extends"" 子句。","""extends"" 子句必须位于 ""implements"" 子句之前。","导出的类“{0}”的 ""extends"" 子句具有或正在使用专用名称“{1}”。","导出的类的 ""extends"" 子句具有或正在使用专用名称“{0}”。","导出接口“{0}”的 ""extends"" 子句具有或正在使用专用名称“{1}”。","""false""，除非设置了 ""composite""","""false""，除非设置了 ""strict""",文件,仅当文件是模块且没有导入或导出项时，才允许在该文件的顶层使用“for await”循环。可考虑添加空的“export {}”将此文件变为模块。,仅允许在异步函数和模块顶层使用“for await”循环。,"""for await"" 循环不能在类静态块内使用。","""get"" 和 ""set"" 访问器无法声明 ""this"" 参数。","如果指定了 ""files""，则为 ""[]""，否则为""[""**/*""]5D;""","已看到 ""implements"" 子句。","""implements"" 子句只能在 TypeScript 文件中使用。","""import ... ="" 只能在 TypeScript 文件中使用。","仅条件类型的 ""extends"" 子句中才允许 ""infer"" 声明。","""\k"" 后面必须有一个括在尖括号中的捕获组名称。","""let"" 不能用作 ""let"" 或 ""const"" 声明中的名称。",module === 'AMD' 或 'UMD' 或 'System' 或 'ES6'，然后 'Classic'， 否则为 'Node',"module === ""system"" 或 esModuleInterop","其目标缺少构造签名的 ""new"" 表达式隐式具有 ""any"" 类型。","`[""node_modules""、""bower_components""、""jspm_packages""]`，以及 ""outDir"" 的值(如果指定)。",以下其中一个:,一个或更多:,选项,应为 “{” 或 JSX 元素。,"应为 ""{"" 或 "";""。","""package.json"" 没有“{0}”字段。","""package. json"" 没有与版本 ""{0}"" 匹配的 ""typesVersions"" 项。","""package. json"" 具有错误的 ""{0}"" 字段。","""package.json"" 具有引用“{2}”的“{0}”字段“{1}”。","""package.json"" 具有 ""peerDependencies"" 字段。","""package. json"" 具有 ""typesVersions"" 项 ""{0}""，它不是有效的 semver 范围。","""package. json"" 具有与编译器版本 ""{1}"" 匹配的 ""typesVersions"" 项 ""{0}""，它需要与模块名称 ""{2}"" 匹配的模式。","""package. json"" 具有 ""typesVersions"" 字段，它具有特定于版本的路径映射。",package.json 作用域 '{0}' 将说明符 '{1}' 显式映射到 NULL。,package.json 作用域 '{0}' 的说明符 '{1}' 的目标类型无效,package.json 作用域 '{0}' 未定义导入。,"指定了 ""paths“ 选项，正在查找模式以匹配模块名“{0}”。","""\q"" 仅在字符类内可用。","""\q"" 后面必须是括在大括号中的字符串替代项。","""readonly"" 修饰符仅可出现在属性声明或索引签名中。","仅允许对数组和元组字面量类型使用 ""readonly"" 类型修饰符。","可将 ""require"" 调用转换为 import 语句。","只能为仅类型导入设置 ""resolution-mode""。","""resolution-mode"" 是类型导入断言的唯一有效密钥。","""resolution-mode"" 是类型导入属性的唯一有效键。",“resolution-mode”应为“require”或“import”。,"设置了 ""rootDirs"" 选项，可将其用于解析相对模块名称“{0}”。","只能在派生类中引用 ""super""。","仅可在派生类或对象字面量表达式的成员中引用 ""super""。","不能在计算属性名中引用 ""super""。","不能在构造函数参数中引用 ""super""。","""target"" 选项为 ""ES2015"" 或更高版本时，仅对象字面量表达式的成员中允许 ""super""。","""super"" 不能使用类型参数。","访问派生类构造函数中的 ""super"" 属性前，必须调用 ""super""。","访问派生类的构造函数中的 ""this"" 前，必须调用 ""super""。","""super"" 的后面必须是参数列表或成员访问。","只有构造函数、成员函数或派生类的成员访问器中才允许 ""super"" 属性访问。","不能在计算属性名中引用 ""this""。","不能在模块或命名空间体中引用 ""this""。","不能在静态属性初始化表达式中引用 ""this""。","不能在当前位置引用 ""this""。","""this"" 隐式具有类型 ""any""，因为它没有类型注释。",对于 ES2022 及更高版本为 `true`，包括 ESNext。,"如果为 ""composite""，则为 ""true""，否则为 ""false""","当 ""moduleResolution"" 为 ""node16""、""nodenext ""或 ""bundler"" 时为 `true`；否则为 `false`。",tsc: TypeScript 编译器,类型:,"此处不允许使用 ""unique symbol"" 类型。","""unique symbol"" 类型仅可用于变量语句中的变量。","不可在具有绑定名称的变量声明中使用 ""unique symbol"" 类型。","""use strict"" 指令不能与非简单参数列表一起使用。","此处使用了 ""use strict"" 指令。",环境上下文中不允许使用 ‘using’ 声明。,"不允许在异步函数块中使用 ""with"" 语句。","严格模式下不允许使用 ""with"" 语句。","""yield"" 表达式隐式导致 ""any"" 类型，因为它的包含生成器缺少返回类型批注。","不能在参数初始化表达式中使用 ""yield"" 表达式。"
